...
function baseDoc(
  ...
  sections: ReportDocument['sections'],
  subtitle?: string,
): ReportDocument {
  return {
    title,
...
// ─── 1) Expense Breakdown ────────────────────────────────────────────────────
export function buildExpenseBreakdownReport(
  data: ExpenseBreakdownReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  ...
  const L = fr
    ...
    : {
        ...
        share: 'Share',
        amount: 'Amount',
        byCat: 'Operating expenses',
        empty: 'No expenses in this period.',
      }
  const change =
    data.changePct !== 0
      ? {
          text: `${data.changePct > 0 ? '▲' : '▼'} ${pct(Math.abs(data.changePct))}`,
          tone: (data.changePct > 0 ? 'down' : 'up') as 'down' | 'up',
        }
      : null
  const document = baseDoc(L.title, opts, [
    {
      ...
      items: [
        ...
        {
          label: L.pending,
          value: m(data.pendingAmount),
          hint: n(data.pendingCount),
  ...
  const csv = toCsv(
    [L.cat, L.share, L.amount],
    data.byCategory.map((c) => [c.name, String(c.percentage), String(c.amount)]),
  )
...
export function buildStockValuationReport(
  ...
  const L = fr
    ? {
        ...
        sku: 'SKU',
        qty: 'Qté',
        value: 'Valeur au coût',
        other: 'Autres articles',
        total: 'TOTAL AU COÛT',
        empty: 'Aucun stock suivi.',
      }
    : {
        title: 'Stock Valuation',
        skus: 'Tracked SKUs',
        cost: 'Value at cost',
        retail: 'Value at retail',
...
export function buildLowStockReport(
  ...
  const L = fr
    ? {
        title: 'Stocks faibles & réapprovisionnement',
        count: 'Articles à commander',
        estCost: 'Coût estimé',
        product: 'Produit',
        sku: 'SKU',
        onHand: 'En stock',
        ...
        cost: 'Coût est.',
        total: 'TOTAL RÉAPPRO.',
        empty: 'Tous les niveaux de stock sont sains.',
      }
    : {
        title: 'Low Stock & Reorder',
        count: 'Items to reorder',
        estCost: 'Est. reorder cost',
        product: 'Product',
        sku: 'SKU',
        onHand: 'On hand',
        level: 'Reorder lvl',
...
export function buildAgeingReport(
  ...
  const L = fr
    ? {
        recvTitle: 'Balance âgée clients',
        payTitle: 'Balance âgée fournisseurs',
        asAt: 'Au',
        account: 'Compte',
        cur: 'Courant (≤7 j)',
        b1: '8–15 j',
        ...
        total: 'Total',
        totalRow: 'TOTAL',
        outstanding: 'Total en cours',
        overdue: 'En retard (30 j+)',
        empty: 'Aucun solde en cours.',
        noteR:
          'Les montants de plus de 30 jours doivent être suivis en priorité pour recouvrement.',
        noteP:
          'Priorisez le règlement du courant et du 8–15 j afin de préserver vos conditions fournisseurs.',
      }
    : {
        recvTitle: 'Aged Receivables',
        payTitle: 'Aged Payables',
        asAt: 'As at',
        account: 'Account',
        cur: 'Current (≤7 d)',
        b1: '8–15 d',
    ...
    sections: [
      {
        ...
        items: [
          {
            label: L.outstanding,
            value: m(t.totalOutstanding),
            tone: kind === 'receivable' ? 'up' : 'down',
          },
  ...
  const csvRows = report.entries.map((e) => [
    e.contactName,
    String(e.current),
    String(e.moderate),
    String(e.aged),
    String(e.overdue),
    String(e.totalOutstanding),
  ])
  csvRows.push([
    L.totalRow,
    String(t.current),
...
export function buildDailySalesReport(
  data: DailySalesSeriesReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  ...
  const L = fr
    ? {
        title: 'Résumé des ventes journalières',
        totalSales: 'Ventes totales',
        txns: 'Transactions',
        avgDay: 'Moy. / jour',
        basket: 'Panier moyen',
        days: 'jours ouvrés',
        perTxn: 'par transaction',
        date: 'Date',
        cash: 'Espèces',
        momo: 'Mobile Money',
        card: 'Carte',
...
export function buildCashierPerformanceReport(
  data: CashierPerformanceReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        ...
        discounts: 'Remises',
        totalRow: 'TOTAL / ÉQUIPE',
        sec: "Par membre de l'équipe",
        empty: 'Aucune vente sur cette période.',
        note: 'Les remboursements et remises sont affichés à titre de contrôle. Un taux de remboursement élevé peut justifier une revue de poste.',
      }
    : {
        ...
        sec: 'By team member',
        empty: 'No sales in this period.',
        note: 'Refunds and discounts are shown for oversight. High refund ratios may warrant a shift review.',
      }
...
export function buildSalesByProductReport(
  data: SalesByProductReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        ...
        marginPct: 'Marge %',
        total: 'TOTAL',
        sec: 'Performance par article',
        empty: 'Aucune vente sur cette période.',
        note: 'Classé par chiffre d’affaires. Coût au CMUP ; la marge exclut les charges d’exploitation indirectes.',
      }
    : {
        ...
        empty: 'No sales in this period.',
        note: 'Ranked by revenue. COGS at weighted-average cost; margin excludes indirect operating charges.',
      }
  ...
  const document = baseDoc(L.title, opts, [
    {
      ...
      items: [
        { label: L.rev, value: m(revenue), hint: `${n(data.rows.length)} ${L.lines}` },
        {
          label: L.margin,
          value: m(margin),
          hint: `${pct(revenue ? (margin / revenue) * 100 : 0)} ${L.ofRev}`,
          tone: 'up',
        },
    ...
    },
    {
      ...
      rows: data.rows.map((r) => {
        ...
        return {
          ...
          cogs: m(r.cogs),
          margin: m(mg),
          marginPct: pct(r.revenue ? (mg / r.revenue) * 100 : 0),
        }
      }),
      total: {
        product: L.total,
        ...
        cogs: m(cogs),
        margin: m(margin),
        marginPct: pct(revenue ? (margin / revenue) * 100 : 0),
      },
      empty: L.empty,
    },
    { kind: 'note', text: L.note },
  ...
  const csv = toCsv(
    ...
    data.rows.map((r) => {
      ...
      return [
        ...
        String(r.cogs),
        String(mg),
        pct(r.revenue ? (mg / r.revenue) * 100 : 0),
      ]
    }),
  )
  return { document, csv }
...
export function buildSalesByCategoryReport(
  data: SalesByCategoryReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const document = baseDoc(L.title, opts, [
    {
      ...
      items: [
        { label: L.rev, value: m(revenue), hint: `${n(data.rows.length)} ${L.cats}` },
        {
          label: L.margin,
          value: m(margin),
          hint: pct(revenue ? (margin / revenue) * 100 : 0),
          tone: 'up',
        },
        {
          label: L.top,
          value: top ? top.category : '—',
          hint: top && revenue ? `${pct((top.revenue / revenue) * 100)} ${L.ofSales}` : '',
        },
    ...
    },
    {
      ...
      rows: data.rows.map((r) => {
        ...
        return {
          ...
          margin: m(mg),
          marginPct: pct(r.revenue ? (mg / r.revenue) * 100 : 0),
          pctSales: pct(revenue ? (r.revenue / revenue) * 100 : 0),
        }
      }),
      total: {
        cat: L.total,
...
export function buildSalesByPaymentReport(
  data: SalesByPaymentReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  const n = (x: number) => formatNumber(x, opts.locale)
  const methodLabel = (mth: string) =>
    PAYMENT_LABELS[mth] ? (fr ? PAYMENT_LABELS[mth].fr : PAYMENT_LABELS[mth].en) : mth
  const L = fr
    ? {
        ...
        fees: 'Frais est.',
        total: 'TOTAL',
        sec: 'Répartition des encaissements',
        empty: 'Aucun encaissement sur cette période.',
        note: 'Les frais Mobile Money & carte sont estimés aux taux courants au Cameroun (MoMo ~1 %, carte ~1,8 %). Les espèces et ventes à crédit ne supportent aucun frais.',
      }
    : {
        ...
        note: 'Mobile Money & card fees are estimated at typical Cameroon rates (MoMo ~1%, card ~1.8%). Cash and on-account sales incur no processing fee.',
      }
  const total = data.rows.reduce((s, r) => s + r.amount, 0)
  const txns = data.rows.reduce((s, r) => s + r.transactions, 0)
  const feeOf = (r: { method: string; amount: number }) =>
    Math.round(r.amount * (PAYMENT_FEE_RATE[r.method] ?? 0))
  ...
  const document = baseDoc(L.title, opts, [
    {
      ...
      rows: data.rows.map((r) => ({
        ...
        txns: n(r.transactions),
        amount: m(r.amount),
        share: pct(total ? (r.amount / total) * 100 : 0),
        fees: m(feeOf(r)),
      })),
      total: {
        method: L.total,
...
export function buildRefundsReport(
  data: RefundsReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        ...
        value: 'Valeur remboursée',
        rate: 'Taux de remboursement',
        avg: 'Remb. moyen',
        topReason: 'Motif principal',
        txns: 'transactions',
        ofGross: 'des ventes brutes',
        ...
        other: 'Autres / non précisé',
        empty: 'Aucun remboursement sur cette période.',
        note: 'Un taux de remboursement supérieur à ~2 % des ventes brutes mérite examen. Les produits abîmés/périmés révèlent un problème de rotation ; surveillez les taux élevés par caissier.',
      }
    : {
        ...
        note: 'A refund rate above ~2% of gross sales warrants review. Damaged / expired goods point to stock rotation; investigate high per-cashier rates.',
      }
  ...
  const document = baseDoc(L.title, opts, [
      ...
      items: [
        {
          label: L.value,
          value: m(totAmt),
          hint: `${n(totCount)} ${L.txns}`,
          tone: totAmt > 0 ? 'warn' : undefined,
        },
        {
          label: L.rate,
          value: pct(data.grossSales ? (totAmt / data.grossSales) * 100 : 0),
          hint: L.ofGross,
        },
        { label: L.avg, value: m(totCount ? Math.round(totAmt / totCount) : 0), hint: L.perReturn },
        {
          ...
          hint: topReason && totAmt ? pct((topReason.amount / totAmt) * 100) : '',
        },
      ],
    },
    {
      ...
      rows: data.byReason.map((r) => ({
        ...
        count: n(r.count),
        amount: m(r.amount),
        pctValue: pct(totAmt ? (r.amount / totAmt) * 100 : 0),
      })),
    ...
    },
    {
      ...
      rows: data.byCashier.map((c) => ({
        ...
        refunds: m(c.refunds),
        sales: m(c.sales),
        rate: pct(c.sales ? (c.refunds / c.sales) * 100 : 0),
      })),
      total: {
        member: L.total,
        ...
        rate: pct(
          data.byCashier.reduce((s, c) => s + c.sales, 0)
            ? (data.byCashier.reduce((s, c) => s + c.refunds, 0) /
                data.byCashier.reduce((s, c) => s + c.sales, 0)) *
                100
            : 0,
        ),
  ...
  const csv = toCsv(
    ...
    data.byReason.map((r) => [
      ...
      String(r.count),
      String(r.amount),
      pct(totAmt ? (r.amount / totAmt) * 100 : 0),
    ]),
  )
  return { document, csv }
}
...
export function buildIncomeStatementReport(
  ...
  const L = fr
    ? {
        ...
        metric: 'Poste',
        value: 'Montant',
        noExp: 'Aucune charge enregistrée sur la période.',
        note: "Compte de résultat de gestion établi à partir des ventes du point de vente et des charges enregistrées. Ce n'est pas un état financier statutaire SYSCOHADA complet (qui nécessite le grand livre : amortissements, charges financières, impôt).",
      }
    : {
        ...
        metric: 'Line',
        value: 'Amount',
        noExp: 'No expenses recorded in the period.',
        note: 'A management income statement from point-of-sale revenue and recorded expenses. It is not a full statutory SYSCOHADA statement (which needs the general ledger: depreciation, financial charges, tax).',
      }
...
export function buildInventoryTurnoverReport(
  data: InventoryTurnoverReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  const L = fr
    ? {
        ...
        overall: 'Rotation globale',
        days: 'Jours de couverture',
        avg: 'Stock moyen',
        cogs: 'CMV annuel',
        ofCost: 'au coût',
        cogsH: 'coût des ventes',
        product: 'Produit',
        avgC: 'Stock (coût)',
        cogsC: 'CMV annuel',
        turn: 'Rotation',
        ...
        speed: 'Vitesse',
        portfolio: 'ENSEMBLE',
        sec: 'Rotation par article',
        empty: 'Aucun article suivi.',
        fast: 'Rapide',
        steady: 'Régulier',
        slow: 'Lent',
        note: 'Rotation = CMV annuel ÷ stock moyen au coût. Couverture = 365 ÷ rotation. Les articles lents (>90 j) immobilisent la trésorerie — voir le rapport Stocks dormants.',
      }
    : {
        title: 'Inventory Turnover',
        overall: 'Overall turnover',
        days: 'Days on hand',
        avg: 'Avg inventory',
        cogs: 'Annual COGS',
        ofCost: 'at cost',
        cogsH: 'cost of goods sold',
        product: 'Product',
        avgC: 'Avg stock (cost)',
        cogsC: 'Annual COGS',
        turn: 'Turnover',
        ...
        speed: 'Speed',
        portfolio: 'PORTFOLIO',
        sec: 'Turnover by product',
        empty: 'No tracked items.',
        fast: 'Fast',
        steady: 'Steady',
        slow: 'Slow',
        note: 'Turnover = annual COGS ÷ average inventory at cost. Days on hand = 365 ÷ turnover. Slow items (>90 days) tie up cash — see the Dead / Slow-Moving Stock report.',
      }
  ...
  const turnOf = (r: { avgStockCost: number; annualCogs: number }) =>
    r.avgStockCost > 0 ? r.annualCogs / r.avgStockCost : 0
  const daysOf = (tn: number) => (tn > 0 ? Math.round(365 / tn) : 0)
  const speedOf = (dd: number, tn: number) =>
    tn <= 0 ? L.slow : dd <= 45 ? L.fast : dd <= 90 ? L.steady : L.slow
  ...
  const document = baseDoc(L.title, opts, [
    ...
    },
    {
      ...
      total: {
        ...
        avg: m(avgTot),
        cogs: m(cogsTot),
        turn: `${overall.toFixed(1)}×`,
        days: overallDays > 0 ? `${overallDays} d` : '—',
        speed: '',
      },
      empty: L.empty,
    },
...
export function buildDeadStockReport(
  ...
  const L = fr
    ? {
        title: 'Stocks dormants & à rotation lente',
        cash: 'Trésorerie immobilisée',
        critical: 'Critiques (90 j+)',
        oldest: 'Le plus ancien',
        pctInv: '% du stock',
        items: 'articles au coût',
        clear: 'à écouler',
        sinceSale: 'depuis la dernière vente',
        ofCost: 'du stock au coût',
        product: 'Produit',
        sku: 'SKU',
        qty: 'Qté',
        cost: 'Valeur au coût',
        ...
        clearNow: 'Écouler',
        discount: 'Solder',
        total: 'TOTAL IMMOBILISÉ',
        sec: 'Articles sans vente récente',
        empty: 'Aucun stock dormant — tout tourne.',
        note: 'Envisagez promotions, lots ou déstockage au-delà de 90 jours. Priorisez les périssables proches de péremption pour éviter une perte (charge HAO).',
      }
    : {
        title: 'Dead & Slow-Moving Stock',
        cash: 'Cash tied up',
        critical: 'Critical (90+ d)',
        oldest: 'Oldest',
        pctInv: '% of inventory',
        items: 'items at cost',
        clear: 'to clear',
        sinceSale: 'since last sale',
        ofCost: 'of stock at cost',
        product: 'Product',
        sku: 'SKU',
        qty: 'Qty',
        cost: 'Cost value',
        ...
        clearNow: 'Clear now',
        discount: 'Discount',
        total: 'TOTAL CASH TIED UP',
        sec: 'Items without recent sales',
        empty: 'No dead stock — everything is moving.',
        note: 'Consider promotions, bundling or clearance for items past 90 days. Prioritise perishables near expiry to avoid a write-off (HAO charge).',
      }
  const totalCost = data.rows.reduce((s, r) => s + r.costValue, 0)
  const critical = data.rows.filter(
  ...
  const sorted = data.rows
    .slice()
    .sort((a, b) => (b.daysSinceLastSale ?? 1e9) - (a.daysSinceLastSale ?? 1e9))
  const document = baseDoc(L.title, opts, [
    {
      kind: 'kpis',
      items: [
        {
          label: L.cash,
          value: m(totalCost),
          hint: `${n(data.rows.length)} ${L.items}`,
          tone: data.rows.length > 0 ? 'warn' : 'up',
        },
        {
          label: L.critical,
          value: `${n(critical)}`,
          hint: L.clear,
          tone: critical > 0 ? 'down' : undefined,
        },
        { label: L.oldest, value: oldest > 0 ? `${n(oldest)} d` : '—', hint: L.sinceSale },
        {
          label: L.pctInv,
          value: pct(data.stockCostTotal ? (totalCost / data.stockCostTotal) * 100 : 0),
          hint: L.ofCost,
        },
      ],
    },
  ...
  const csv = toCsv(
    ...
    sorted.map((r) => [
      ...
      String(r.quantity),
      String(r.costValue),
      r.daysSinceLastSale === null ? L.never : String(r.daysSinceLastSale),
    ]),
  )
  return { document, csv }
}
...
export function buildSupplierPriceReport(
  data: SupplierPriceReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  const L = fr
    ? {
        title: 'Évolution des prix fournisseurs',
        product: 'Produit',
        ...
        cur: 'Actuel',
        change: 'Variation',
        sec: "Coût d'achat unitaire",
        empty: 'Aucun historique de réapprovisionnement.',
        note: 'La hausse des coûts (▲) érode les marges si les prix de vente ne suivent pas. Revoyez la tarification des articles au-delà de +3 %.',
      }
    : {
        ...
        cur: 'Current',
        change: 'Change',
        sec: 'Unit purchase cost',
        empty: 'No restock history.',
        note: 'Rising costs (▲) erode margins unless retail prices follow. Review pricing on items above +3%.',
      }
  ...
  const csv = toCsv(
    ...
    data.rows.map((r) => [
      r.name,
      r.supplier ?? '',
      r.cost6mo === null ? '' : String(r.cost6mo),
      r.cost3mo === null ? '' : String(r.cost3mo),
      r.current === null ? '' : String(r.current),
      changeOf(r),
    ]),
  )
  return { document, csv }
...
export function buildStockMovementsReport(
  data: StockMovementsReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const n = (x: number) => formatNumber(x, opts.locale)
  ...
  const typeLabel = (t: string) =>
    MOVEMENT_LABELS[t] ? (fr ? MOVEMENT_LABELS[t].fr : MOVEMENT_LABELS[t].en) : t
  const when = (iso: string) => `${iso.slice(0, 10)} ${iso.slice(11, 16)}`.trim()
  const L = fr
    ? {
        title: 'Mouvements de stock',
        moves: 'Mouvements',
        unitsIn: 'Unités entrées',
        unitsOut: 'Unités sorties',
        net: 'Variation nette',
        date: 'Date',
        product: 'Produit',
        type: 'Type',
        ...
        trunc: 'Seuls les mouvements les plus récents sont affichés.',
      }
    : {
        ...
        trunc: 'Only the most recent movements are shown.',
      }
...
const REASON_LABELS: Record<string, { en: string; fr: string }> = {
  NEGOTIATED: { en: 'Negotiated', fr: 'Négocié' },
  REGULAR_CUSTOMER: { en: 'Regular customer', fr: 'Client régulier' },
  BULK: { en: 'Bulk', fr: 'En gros' },
  DAMAGED: { en: 'Damaged', fr: 'Endommagé' },
  NEAR_EXPIRY: { en: 'Near expiry', fr: 'Proche péremption' },
  STAFF_PURCHASE: { en: 'Staff purchase', fr: 'Achat personnel' },
  ROUNDING: { en: 'Rounding', fr: 'Arrondi' },
  OTHER: { en: 'Other', fr: 'Autre' },
}
const reasonLabel = (code: string | null, fr: boolean): string =>
  code ? (REASON_LABELS[code]?.[fr ? 'fr' : 'en'] ?? code) : fr ? 'Non précisé' : 'Unspecified'

export function buildDiscountSummaryReport(
  data: DiscountSummaryReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        title: 'Résumé des remises',
        total: 'Total des remises',
        ofSales: 'des ventes',
        discounted: 'Ventes remisées',
        ofCount: 'sur',
        flagged: 'À autoriser',
        belowCost: 'Sous le coût',
        reason: 'Motif',
        count: 'Nombre',
        amount: 'Montant',
        sec: 'Par motif',
        empty: 'Aucune remise sur cette période.',
        note: 'La remise est exprimée en part du chiffre d’affaires net. « À autoriser » = remises hors limite non validées par un responsable.',
      }
    : {
        title: 'Discount Summary',
        total: 'Total discounts',
        ofSales: 'of sales',
        discounted: 'Discounted sales',
        ofCount: 'of',
        ...
        note: 'Discount rate is shown as a share of net sales revenue. “Needs approval” = over-limit discounts not authorized by a manager.',
      }
  const rate = data.grossSales > 0 ? (data.totalDiscount / data.grossSales) * 100 : 0
  const document = baseDoc(L.title, opts, [
    {
      kind: 'kpis',
      items: [
        {
          label: L.total,
          value: m(data.totalDiscount),
          hint: `${pct(rate)} ${L.ofSales}`,
          tone: 'down',
        },
        {
          label: L.discounted,
          value: n(data.discountedSaleCount),
          hint: `${L.ofCount} ${n(data.saleCount)}`,
        },
        {
          label: L.flagged,
          value: n(data.unauthorizedCount),
          tone: data.unauthorizedCount > 0 ? 'warn' : undefined,
        },
        {
          label: L.belowCost,
          value: n(data.belowCostCount),
          tone: data.belowCostCount > 0 ? 'warn' : undefined,
        },
      ],
    },
    {
      ...
      rows: data.byReason.map((r) => ({
        reason: reasonLabel(r.reasonCode, fr),
        count: n(r.count),
        amount: m(r.amount),
      })),
      total: {
        reason: fr ? 'TOTAL' : 'TOTAL',
        count: n(data.byReason.reduce((s, r) => s + r.count, 0)),
        amount: m(data.totalDiscount),
      },
      empty: L.empty,
    },
    { kind: 'note', text: L.note },
  ...
  document.unit = data.currency
  const csv = toCsv(
    [L.reason, L.count, L.amount],
    data.byReason.map((r) => [reasonLabel(r.reasonCode, fr), String(r.count), String(r.amount)]),
  )
  return { document, csv }
}

export function buildDiscountByCashierReport(
  data: DiscountByCashierReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        ...
        rate: 'Taux',
        flagged: 'À autoriser',
        sec: 'Classé par taux de remise',
        total: 'TOTAL',
        empty: 'Aucune remise sur cette période.',
        note: 'L’anomalie à surveiller est l’écart : un caissier à 9 % quand les autres sont à 2 %. Recoupez avec l’écart de caisse avant de conclure.',
      }
    : {
        title: 'Discounts by Cashier',
        cashier: 'Cashier',
        disc: 'Discounts',
        sales: 'Sales',
        rate: 'Rate',
        flagged: 'Needs approval',
        sec: 'Ranked by discount rate',
        total: 'TOTAL',
        empty: 'No discounts in this period.',
        note: 'The signal is the outlier — one cashier at 9% when the rest are at 2%. Corroborate with cash variance before drawing a conclusion.',
      }
  const rows = [...data.rows].sort((a, b) => {
    const ra = a.grossSales > 0 ? a.discountTotal / a.grossSales : 0
    const rb = b.grossSales > 0 ? b.discountTotal / b.grossSales : 0
    return rb - ra
  })
  const totDisc = rows.reduce((s, r) => s + r.discountTotal, 0)
  const totGross = rows.reduce((s, r) => s + r.grossSales, 0)
  const totFlag = rows.reduce((s, r) => s + r.unauthorizedCount, 0)
  const document = baseDoc(L.title, opts, [
    {
      kind: 'table',
      title: L.sec,
      columns: [
        ...
        { key: 'disc', label: L.disc, align: 'right' },
        { key: 'sales', label: L.sales, align: 'right' },
        { key: 'rate', label: L.rate, align: 'right' },
        { key: 'flagged', label: L.flagged, align: 'right' },
      ],
      rows: rows.map((r) => ({
        cashier: r.cashierName,
        disc: m(r.discountTotal),
        sales: m(r.grossSales),
        rate: pct(r.grossSales > 0 ? (r.discountTotal / r.grossSales) * 100 : 0),
        flagged: r.unauthorizedCount > 0 ? n(r.unauthorizedCount) : '—',
      })),
      total: {
        cashier: L.total,
        disc: m(totDisc),
        sales: m(totGross),
        rate: pct(totGross > 0 ? (totDisc / totGross) * 100 : 0),
        flagged: totFlag > 0 ? n(totFlag) : '—',
      },
      empty: L.empty,
    },
    { kind: 'note', text: L.note },
  ...
  document.unit = data.currency
  const csv = toCsv(
    [L.cashier, L.disc, L.sales, L.rate, L.flagged],
    rows.map((r) => [
      r.cashierName,
      String(r.discountTotal),
      String(r.grossSales),
      pct(r.grossSales > 0 ? (r.discountTotal / r.grossSales) * 100 : 0),
      String(r.unauthorizedCount),
    ]),
  )
  return { document, csv }
}

export function buildDiscountByProductReport(
  data: DiscountByProductReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  ...
  const L = fr
    ? {
        ...
        margin: 'Marge',
        marginPct: 'Marge %',
        sec: 'Produits les plus remisés',
        total: 'TOTAL',
        empty: 'Aucune remise sur cette période.',
        note: 'Un article toujours remisé signale le plus souvent un prix affiché trop haut — c’est une correction de prix, pas une fraude. La marge est calculée après remise.',
      }
    : {
        ...
        margin: 'Margin',
        marginPct: 'Margin %',
        sec: 'Most-discounted products',
        total: 'TOTAL',
        empty: 'No discounts in this period.',
        note: 'A product that’s always discounted usually means its list price is too high — a pricing fix, not a fraud finding. Margin is shown after discount.',
      }
  const rows = [...data.rows].sort((a, b) => b.discountTotal - a.discountTotal)
  const totDisc = rows.reduce((s, r) => s + r.discountTotal, 0)
  const totRev = rows.reduce((s, r) => s + r.revenue, 0)
  const totCogs = rows.reduce((s, r) => s + r.cogs, 0)
  const totMargin = totRev - totCogs
  const document = baseDoc(L.title, opts, [
    {
      kind: 'table',
      title: L.sec,
      columns: [
        ...
        { key: 'cat', label: L.cat },
        { key: 'disc', label: L.disc, align: 'right' },
        { key: 'times', label: L.times, align: 'right' },
        { key: 'revenue', label: L.revenue, align: 'right' },
        { key: 'margin', label: L.margin, align: 'right' },
        { key: 'marginPct', label: L.marginPct, align: 'right' },
      ],
      rows: rows.map((r) => {
        const mg = r.revenue - r.cogs
        return {
          ...
          cat: r.category ?? '—',
          disc: m(r.discountTotal),
          times: n(r.discountCount),
          revenue: m(r.revenue),
          margin: m(mg),
          marginPct: pct(r.revenue > 0 ? (mg / r.revenue) * 100 : 0),
        }
      }),
      total: {
        product: L.total,
        ...
        revenue: m(totRev),
        margin: m(totMargin),
        marginPct: pct(totRev > 0 ? (totMargin / totRev) * 100 : 0),
      },
      empty: L.empty,
    },
    { kind: 'note', text: L.note },
  ...
  document.unit = data.currency
  const csv = toCsv(
    [L.product, L.cat, L.disc, L.times, L.revenue, L.margin, L.marginPct],
    rows.map((r) => {
      const mg = r.revenue - r.cogs
      return [
        r.name,
        r.category ?? '',
        String(r.discountTotal),
        String(r.discountCount),
        String(r.revenue),
        String(mg),
        pct(r.revenue > 0 ? (mg / r.revenue) * 100 : 0),
      ]
    }),
  )
  return { document, csv }
...
export function buildFlaggedDiscountsReport(
  data: FlaggedDiscountReportData,
  opts: ReportBuildOptions,
): BuiltReportResult {
  const fr = isFr(opts.locale)
  const m = (x: number) => formatMoney(x, data.currency, opts.locale)
  const when = (iso: string): string => {
    try {
      return new Date(iso).toLocaleDateString(opts.locale, {
        day: '2-digit',
        month: 'short',
        year: 'numeric',
      })
  ...
  const L = fr
    ? {
        ...
        unauth: 'Non autorisé',
        below: 'Sous le coût',
        both: 'Non autorisé · sous le coût',
        sec: 'Non autorisées et ventes à perte',
        empty: 'Rien à signaler sur cette période — aucune remise hors limite ni vente à perte.',
        note: 'Les plus récentes en premier. « Non autorisé » = au-delà de la limite du rôle sans validation d’un responsable.',
      }
    : {
        title: 'Flagged Discounts',
        when: 'Date',
        sale: 'Sale',
        cashier: 'Cashier',
        amount: 'Discount',
        ...
        unauth: 'Unauthorized',
        below: 'Below cost',
        both: 'Unauthorized · below cost',
        sec: 'Unauthorized and below-cost sales',
        empty: 'Nothing flagged this period — no over-limit or below-cost sales.',
        note: 'Most recent first. “Unauthorized” = beyond the role limit without a manager’s approval.',
      }
  const flagText = (r: FlaggedDiscountRow): string =>
    r.unauthorized && r.belowCost ? L.both : r.unauthorized ? L.unauth : L.below
  const document = baseDoc(L.title, opts, [
    {
      kind: 'table',
      title: L.sec,
  ...
  document.unit = data.currency
  const csv = toCsv(
    [L.when, L.sale, L.cashier, L.amount, L.reason, L.flag],
    data.rows.map((r) => [
      r.soldAt,
      r.saleNumber,
      r.cashierName ?? '',
