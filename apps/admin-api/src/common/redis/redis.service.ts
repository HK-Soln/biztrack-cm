...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  private createClient(source: 'module_init' | 'lazy_client'): Redis {
    ...
    client.on('connect', () => {
      this.logger.log('Redis socket connected', 'RedisService', { source, status: client.status })
    })
    client.on('ready', () => {
      this.logger.log('Redis client ready', 'RedisService', { source, status: client.status })
    })
    client.on('reconnecting', () => {
      this.logger.warn('Redis client reconnecting', 'RedisService', { source, status: client.status })
    })
    client.on('end', () => {
      this.logger.warn('Redis connection ended', 'RedisService', { source, status: client.status })
    })
