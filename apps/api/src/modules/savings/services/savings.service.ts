...
const accountNumberFor = (id: string): string =>
  `DEP-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${id.slice(0, 4).toUpperCase()}`
...
export class DepositsService {
  ...
  async buildDepositReceipt(transactionId: string, businessId: string): Promise<DepositReceipt> {
    const tx = await this.savingsTransactionsRepo.findOne({
  ...
  async create(
    ...
    // A deposit must belong to a real customer — validate it and denormalize the customer's
    // name/phone onto the account (mirrors the desktop, which reads the name from the contact).
    const contact = await this.dataSource
      .getRepository(Contact)
      .findOne({ where: { id: dto.customerId, businessId } })
    if (!contact) throw new AppNotFoundException('Customer not found.', 'CONTACT_NOT_FOUND')
    const open = await this.savingsAccountsRepo.findOne({
      where: { businessId, customerId: dto.customerId, status: 'OPEN', isDeleted: false },
    })
    if (open)
      throw new AppBadRequestException(
        'This customer already has an open deposit session.',
        'DEPOSIT_OPEN_EXISTS',
      )
    const now = new Date()
    const id = newId()
    ...
    if (initial > 0) {
      await this.insertTxn(
        this.savingsTransactionsRepo.manager,
        id,
        businessId,
        {
          type: 'deposit',
          direction: 'inbound',
          amount: initial,
          method: dto.initialDeposit?.method ?? 'CASH',
          mobileMoneyReference: dto.initialDeposit?.mobileMoneyReference,
          notes: dto.initialDeposit?.notes,
  ...
  async list(
    ...
    if (query.search?.trim()) {
      ...
      qb.andWhere(
        new Brackets((w) =>
          w.where('s.customer_name ILIKE :q', { q }).orWhere('s.account_number ILIKE :q', { q }),
        ),
      )
    }
    qb.orderBy(`CASE WHEN s.status = 'OPEN' THEN 0 ELSE 1 END`, 'ASC')
  ...
  async getOpenForCustomer(
    customerId: string,
    businessId: string,
  ): Promise<CustomerDeposit | null> {
    const acc = await this.savingsAccountsRepo.findOne({
      where: { businessId, customerId, status: 'OPEN', isDeleted: false },
    })
  ...
  async getStatement(id: string, businessId: string): Promise<DepositStatement> {
    ...
    const entries: DepositStatementEntry[] = txns.map((t) => {
      ...
      return {
        id: t.id,
        type: t.type as DepositStatementEntry['type'],
        direction: t.direction as DepositStatementEntry['direction'],
        amount: t.amount,
        method: t.method ?? null,
        mobileMoneyReference: t.mobileMoneyReference ?? null,
  ...
  async close(
    ...
    const salesCount = await this.savingsTransactionsRepo.count({
      where: { savingsId: id, businessId, type: 'sale' },
    })
    ...
    await this.dataSource.transaction(async (m) => {
      ...
      if (dto.settlement === 'NONE') {
        if (leftover > 0)
          throw new AppBadRequestException(
            'There is a leftover balance — refund it or transfer it.',
            'DEPOSIT_LEFTOVER',
          )
        ...
        if (!hasSales)
          throw new AppBadRequestException(
            ...
            'DEPOSIT_TRANSFER_NO_SALES',
          )
        if (leftover <= 0)
          throw new AppBadRequestException('Nothing to transfer.', 'DEPOSIT_NOTHING_TO_TRANSFER')
        await this.insertTxn(
          m,
          id,
          businessId,
        ...
        await repo.update(id, {
          balance: 0,
          totalTransferred: round(acc.totalTransferred + leftover),
          updatedAt: now,
        })
        const nid = newId()
        await repo.save(
          repo.create({
            ...
            customerName: acc.customerName ?? null,
            customerPhone: acc.customerPhone ?? null,
            accountNumber: accountNumberFor(nid),
            balance: leftover,
            totalDeposited: leftover,
            totalRefunded: 0,
            totalUsed: 0,
            totalTransferred: 0,
            status: 'OPEN',
      ...
      const outcome = !hasSales
        ? 'REFUNDED'
        : dto.settlement === 'REFUND'
          ? 'COLLECTED_REFUNDED'
          : dto.settlement === 'TRANSFER'
  ...
  private async requireOpen(id: string, businessId: string): Promise<CustomerDeposit> {
    ...
    if (!acc) throw new AppNotFoundException('Deposit session not found.', 'DEPOSIT_NOT_FOUND')
    if (acc.status !== 'OPEN')
      throw new AppBadRequestException('This deposit session is closed.', 'DEPOSIT_CLOSED')
    return acc
  }

  private async withSalesCount(acc: CustomerDeposit): Promise<CustomerDeposit> {
    const salesCount = await this.savingsTransactionsRepo.count({
      where: { savingsId: acc.id, businessId: acc.businessId, type: 'sale' },
    })
