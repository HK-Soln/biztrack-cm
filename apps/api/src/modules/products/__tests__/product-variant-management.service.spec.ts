...
function makeService(
  ...
  const mVariantRepo = {
    create: jest.fn((i: any) => ({ id: 'v-new', ...i })),
    save: jest.fn(async (i: any) => i),
    softDelete: jest.fn(async () => ({})),
    count: jest.fn(async () =>
      opts.existingVariants ? Math.max(opts.existingVariants.length - 1, 0) : 0,
    ),
  ...
  const mInvRepo = {
    ...
    findOne: jest.fn(async () =>
      opts.variantLevelQty !== undefined ? { id: 'lvl-1', quantity: opts.variantLevelQty } : null,
    ),
    update: jest.fn(async () => ({})),
  ...
  const mMovementRepo = {
    count: jest.fn(async () => opts.movementHistory ?? 0),
    create: jest.fn((i: any) => i),
    save: jest.fn(async (i: any) => i),
  }
  const mSerialRepo = {
    find: jest.fn(async () => opts.inStockSerials ?? []),
    update: jest.fn(async () => ({})),
    softDelete: jest.fn(async () => ({})),
    count: jest.fn(async () => (opts.inStockSerials ?? []).length),
  }
...
describe('ProductVariantManagementService', () => {
  describe('addVariant', () => {
    it('non-serialised: creates variant + level and writes a stock-in (OPENING when no history)', async () => {
      const { service, mInvRepo, mMovementRepo, auditService } = makeService({
        variantStockTotal: 5,
        movementHistory: 0,
      })
      await service.addVariant(
        'p1',
        'biz-1',
        { name: 'Black', options: [opt], openingStock: 5 } as any,
        context as any,
      )
      expect(mInvRepo.save).toHaveBeenCalledWith(
        expect.objectContaining({ variantId: 'v-new', quantity: 5 }),
      )
      expect(mMovementRepo.create).toHaveBeenCalledWith(
      ...
      expect(auditService.log).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ action: 'CREATE', entityType: 'product_variant' }),
      )
    })
    ...
    it('non-serialised with prior history: stock-in is a MANUAL_ADJUSTMENT', async () => {
      ...
      await service.addVariant(
        'p1',
        'biz-1',
        { name: 'Blue', options: [opt], openingStock: 3 } as any,
        context as any,
      )
      expect(mMovementRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('serialised: creates the variant at 0 with NO movement (stock comes from serials)', async () => {
      const { service, mInvRepo, mMovementRepo } = makeService({
        product: {
          id: 'p1',
          businessId: 'biz-1',
          trackInventory: true,
    ...
    it('rejects a duplicate option combination', async () => {
      ...
      ;(svc as any).variantOptionsRepo.find = jest.fn(async () => [
        { variantId: 'v1', attributeOptionId: 'o-black' },
      ])
      await expect(
        service.addVariant('p1', 'biz-1', { name: 'Dup', options: [opt] } as any, context as any),
      ).rejects.toMatchObject({ code: 'VARIANT_DUPLICATE_COMBINATION' })
    })
  })

  describe('updateVariant', () => {
    it('edits info with NO movement', async () => {
      const { service, mMovementRepo, auditService } = makeService({
        variant: { id: 'v1', name: 'Black', isActive: true },
      })
      await service.updateVariant('p1', 'v1', 'biz-1', { name: 'Jet Black' } as any, context as any)
      expect(mMovementRepo.save).not.toHaveBeenCalled()
      expect(auditService.log).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ action: 'UPDATE', entityType: 'product_variant' }),
      )
    })
  })
  ...
  describe('removeVariant', () => {
    it('non-serialised: writes off the variant level quantity (stock-out)', async () => {
      const { service, mMovementRepo, mInvRepo, auditService } = makeService({
      ...
      expect(auditService.log).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ action: 'DELETE', entityType: 'product_variant' }),
      )
    })
    ...
    it('serialised: retires the variant’s IN_STOCK serial units and writes off their count', async () => {
      ...
      await service.removeVariant(
        ...
        'v1',
        'biz-1',
        { reason: 'Lost batch' } as any,
        context as any,
      )
      expect(mSerialRepo.softDelete).toHaveBeenCalledTimes(2)
      expect(mMovementRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ quantityChange: -2, notes: 'Lost batch' }),
      )
    })

    it('flips hasVariants false when the last variant is removed', async () => {
      const { service, mProductRepo } = makeService({
        variant: { id: 'v1', name: 'Black' },
        variantLevelQty: 0,
      })
