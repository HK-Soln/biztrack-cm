...
describe('ProductSerialUnitsService', () => {
  describe('add', () => {
    it('adds units and writes an OPENING_STOCK movement when there is no prior history', async () => {
      const { service, serialRepo, movementRepo, auditService } = makeService({
        inStockCount: 0,
        movementCount: 0,
      })
      const created = await service.add(
        ...
        {
          units: [
            { serialNumber: 'SN-A', serialType: 'SERIAL_NUMBER' },
            { serialNumber: 'SN-B', serialType: 'SERIAL_NUMBER' },
          ],
        } as any,
        context as any,
      )
    ...
    it('writes a MANUAL_ADJUSTMENT (stock-in) when history already exists', async () => {
      ...
      await service.add(
        'p1',
        'biz-1',
        { units: [{ serialNumber: 'SN-C', serialType: 'SERIAL_NUMBER' }] } as any,
        context as any,
      )
      expect(movementRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('rejects a serial already in stock', async () => {
      const { service } = makeService({
        existingBySerial: { 'SN-DUP': { id: 'u1', status: 'IN_STOCK', deletedAt: null } },
      })
      await expect(
        service.add(
          'p1',
          'biz-1',
          { units: [{ serialNumber: 'SN-DUP', serialType: 'SERIAL_NUMBER' }] } as any,
          context as any,
        ),
      ).rejects.toMatchObject({ code: 'SERIAL_DUPLICATE_IN_STOCK' })
    })

    it('revives a previously retired (soft-deleted) unit instead of inserting', async () => {
      const { service, serialRepo } = makeService({
        existingBySerial: { 'SN-OLD': { id: 'u9', status: 'DAMAGED', deletedAt: new Date() } },
      })
      ...
      const created = await service.add(
        'p1',
        'biz-1',
        { units: [{ serialNumber: 'SN-OLD', serialType: 'SERIAL_NUMBER' }] } as any,
        context as any,
      )
    ...
    it('requires a variant when the product has variants', async () => {
      ...
      await expect(
        service.add(
          'p1',
          'biz-1',
          { units: [{ serialNumber: 'SN-A', serialType: 'SERIAL_NUMBER' }] } as any,
          context as any,
        ),
      ).rejects.toMatchObject({ code: 'SERIAL_VARIANT_REQUIRED' })
    })

    it('throws when the product is not serialised', async () => {
      const { service } = makeService({
        product: { id: 'p1', businessId: 'biz-1', isSerialized: false },
      })
      await expect(
        service.add(
          'p1',
      ...
      ).rejects.toMatchObject({ code: 'PRODUCT_NOT_SERIALIZED' })
    })
  })
  ...
  describe('retire', () => {
    it('retires an in-stock unit, writes a -1 stock-out movement and audits with the reason', async () => {
      ...
      await service.retire(
        ...
        'u1',
        'biz-1',
        { reason: 'Damaged in transit' } as any,
        context as any,
      )
      ...
      expect(auditService.log).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ action: 'DELETE', entityType: 'product_serial_unit' }),
      )
    })
    ...
    it('refuses to retire a sold unit', async () => {
      ...
      await expect(
        service.retire('p1', 'u1', 'biz-1', { reason: 'whatever' } as any, context as any),
      ).rejects.toMatchObject({ code: 'SERIAL_UNIT_NOT_IN_STOCK' })
    })
  })

  describe('updateSerialNumber', () => {
    it('corrects a serial number WITHOUT writing a movement', async () => {
      const { service, movementRepo, serialRepo, auditService } = makeService({
        unit: {
          id: 'u1',
          productId: 'p1',
          businessId: 'biz-1',
