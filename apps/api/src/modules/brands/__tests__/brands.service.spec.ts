...
function makeService(opts: { brand?: any; model?: any; existingLinks?: any[] } = {}) {
  const brand = opts.brand ?? { id: 'b1', businessId: 'biz-1', name: 'Samsung', slug: 'samsung', isActive: true, sortOrder: 0, models: [], categoryLinks: [] }
  const brandsRepo = {
...
describe('BrandsService', () => {
  it('create: generates a slug, saves the brand, links categories, audits CREATE', async () => {
    ...
    expect(mBrandRepo.save).toHaveBeenCalledWith(expect.objectContaining({ name: 'Samsung', slug: 'samsung' }))
    ...
    expect(auditService.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'CREATE', entityType: 'brand' }))
  })
  ...
  it('update: reconciles category links (adds new, soft-deletes removed) + audits', async () => {
    const { service, mLinkRepo, auditService } = makeService({
      brand: { id: 'b1', businessId: 'biz-1', name: 'Samsung', slug: 'samsung', isActive: true, sortOrder: 0, models: [], categoryLinks: [{ id: 'l1', categoryId: 'c1' }] },
    })
    ...
    expect(auditService.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'UPDATE', entityType: 'brand' }))
  })
  ...
  it('remove: soft-deletes models + links + brand, audits DELETE', async () => {
    ...
    expect(mBrandRepo.update).toHaveBeenCalledWith({ id: 'b1', businessId: 'biz-1' }, expect.objectContaining({ isActive: false }))
    expect(auditService.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'DELETE', entityType: 'brand' }))
  })
  ...
  it('addModel: creates a model under the brand + audits', async () => {
    ...
    expect(modelsRepo.save).toHaveBeenCalledWith(expect.objectContaining({ brandId: 'b1', name: 'Galaxy S24' }))
    expect(auditService.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'CREATE', entityType: 'model' }))
  })
  ...
  it('findById: throws when the brand does not exist', async () => {
    ...
    await expect(service.findById('nope', 'biz-1')).rejects.toMatchObject({ code: 'BRAND_NOT_FOUND' })
  })

  it('removeModel: 404 when the model is not under the brand', async () => {
    const { service } = makeService({ model: null })
    await expect(service.removeModel('b1', 'mX', 'biz-1', ctxArg)).rejects.toMatchObject({ code: 'MODEL_NOT_FOUND' })
  })
})
