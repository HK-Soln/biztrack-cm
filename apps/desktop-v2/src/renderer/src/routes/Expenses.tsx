...
export function Expenses() {
  ...
  const filters = useMemo<ExpensesListQuery>(
    () => ({ ...range, categoryId: category || undefined, search: search.trim() || undefined }),
  ...
  const { data: cats = [] } = useQuery({
    ...
    queryFn: () => dataClient.expenseCategories.listAll(),
    enabled: true,
  })
  const summary = useQuery({
    queryKey: ['expenses', 'summary', range, category, search],
    queryFn: () => dataClient.expenses.summary(filters),
    enabled: true,
  })
  const trend = useQuery({
    queryKey: ['expenses', 'trend'],
    queryFn: () => dataClient.expenses.trend(),
    enabled: true,
  })
  const list = useQuery({
    queryKey: ['expenses', 'list', range, category, search, page],
    queryFn: () => dataClient.expenses.list({ ...filters, page, limit: PAGE }),
    enabled: true,
  })
      ...
      <div className="donut-wrap">
        ...
        <div className="cat-legend">
          ...
          {(s?.byCategory ?? []).length === 0 ? (
            <div className="hint">{t('expenses.noData')}</div>
          ) : null}
        </div>
      </div>
    </div>
      ...
      <>
        <header className="m-head">
          ...
          <div className="m-tt">
            ...
            <div className="m-sub">
              {monthLabel} · {money.compact(s?.total ?? 0)}
            </div>
          </div>
        </header>
        ...
        <div className="mcard" style={{ marginBottom: 18 }}>
          ...
          >
            ...
            {s && s.changePct !== 0 ? (
              <span
                className={`mst ${s.changePct > 0 ? 'mst-out' : 'mst-ok'}`}
                style={{ fontSize: 10.5 }}
              >
                <span className="d" />
          ...
          <div className="stat-row">
            <div className="well">
              <div className="k">{t('expenses.thisMonth')}</div>
              <div className="v">{money.compact(thisMonth)}</div>
            </div>
            <div className="well">
              <div className="k">{t('expenses.avg6')}</div>
              <div className="v">{money.compact(avg6)}</div>
            </div>
            <div className="well">
              <div className="k">{t('expenses.vsLast')}</div>
              <div className="v" style={{ color: change > 0 ? 'var(--danger)' : 'var(--success)' }}>
                {change >= 0 ? '+' : '−'}
                {money.compact(Math.abs(change))}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mlist">
          {list.isPending && rows.length === 0 ? (
            <div className="mrow" style={{ cursor: 'default' }}>
              <div className="mt">
                <div className="sub">{t('expenses.loading')}</div>
              </div>
            </div>
          ) : null}
          {!list.isPending && rows.length === 0 ? (
            <div className="mrow" style={{ cursor: 'default' }}>
              <div className="mt">
                <div className="sub">{t('expenses.empty')}</div>
              </div>
            </div>
          ) : null}
          {rows.map((e) => (
            ...
            >
              ...
              <div className="mt">
                ...
                <div className="sub">
                  {e.categoryName ?? '—'} · {formatDay(e.expenseDate, lang)}
                </div>
              </div>
              <div className="rt">
                <div className="v">{money.format(e.amount)}</div>
                <div className="s">
                  <span className={`mst ${e.status === 'PENDING' ? 'mst-low' : 'mst-ok'}`}>
                    <span className="d" />
                    {e.status === 'PENDING' ? t('expenses.pending') : t('expenses.paid')}
                  </span>
                </div>
              </div>
            </button>
        ...
        {(list.data?.totalPages ?? 1) > 1 ? (
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 14 }}>
            ...
            <button
              ...
              className="mbtn"
              style={{ width: 'auto', padding: '0 18px' }}
              disabled={(list.data?.page ?? 1) >= (list.data?.totalPages ?? 1)}
              onClick={() => setPage((p) => p + 1)}
            >
              {t('expenses.next')}
            </button>
    ...
    <div className="frame">
          ...
          <span className="seg2">
            {(['week', 'month', 'year'] as Period[]).map((p) => (
              <button
                ...
                onClick={() => setPeriod(p)}
              >
                {t(`expenses.${p}` as Parameters<typeof t>[0])}
              </button>
            ))}
          </span>
          <Button
      ...
      <div className="minihead">
        <div className="m">
          <div className="k">
            {t('expenses.kpiTotal')}
            {s && s.changePct !== 0 ? (
              <span className={`badge ${s.changePct > 0 ? 'b-down' : 'b-up'}`}>
                {s.changePct > 0 ? '▲' : '▼'} {Math.abs(s.changePct).toFixed(1)}%
              </span>
            ) : null}
          </div>
          ...
          <div className="h">
            {t('expenses.kpiTotalHint').replace('{prev}', money.compact(s?.previousTotal ?? 0))}
          </div>
        </div>
        <div className="m">
          <div className="k">{t('expenses.kpiLargest')}</div>
          <div className="v" style={{ fontSize: 17 }}>
            {s?.largest ? `${s.largest.name} · ${compactNum(s.largest.amount)}` : '—'}
          </div>
          <div className="h">
            {s?.largest
              ? t('expenses.kpiLargestHint').replace('{pct}', String(s.largest.percentage))
              : ' '}
          </div>
        </div>
        <div className="m">
        ...
        <div className="m">
          <div className="k">
            {t('expenses.kpiPending')}
            {s && s.pendingCount > 0 ? (
              <span className="badge b-warn">{s.pendingCount}</span>
            ) : null}
          </div>
          <div className="v">{money.compact(s?.pendingAmount ?? 0)}</div>
          <div className="h">
            {t('expenses.kpiPendingHint').replace('{n}', String(s?.pendingCount ?? 0))}
          </div>
        </div>
      </div>
      ...
      <div className="split mb20" style={{ alignItems: 'stretch' }}>
        ...
        <div className="card">
          <div className="card-h">
            <div>
              <h3>{t('expenses.trend')}</h3>
              <p>{t('expenses.trendSub')}</p>
            </div>
          </div>
          <div className="bars">
            {trendItems.map((m) => (
            ...
            <tbody>
              {rows.map((e) => (
                ...
                >
                  ...
                  <td className="trunc" style={{ fontWeight: 550 }} title={e.description}>
                    {e.description}
                  </td>
                  <td className="hide-sm">
                    {e.categoryName ? (
                      <span className="chip-tag" style={categoryChipStyle(e.categoryColor)}>
                        {e.categoryName}
                      </span>
                    ) : (
                      '—'
                  ...
                  <td>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                      <span className={`st ${e.status === 'PENDING' ? 'st-low' : 'st-ok'}`}>
                        <span className="d" />
                        {e.status === 'PENDING' ? t('expenses.pending') : t('expenses.paid')}
                      </span>
                      {e.status === 'PENDING' ? (
                        <button
                          type="button"
                          className="markpaid-btn"
                          onClick={(ev) => {
                            ev.stopPropagation()
                            setMarkPaidExpense(e)
                          }}
                        >
                          <svg
                            viewBox="0 0 24 24"
        ...
        <div className="panel-foot">
          <span>
            {t('expenses.foot')
              ...
              .replace('{total}', String(list.data?.total ?? 0))}
          </span>
          ...
          <span>
            {t('expenses.page')
              .replace('{p}', String(list.data?.page ?? 1))
              .replace('{t}', String(list.data?.totalPages ?? 1))}
          </span>
          <span
            className="link"
            aria-disabled={(list.data?.page ?? 1) >= (list.data?.totalPages ?? 1)}
            onClick={() => {
              if ((list.data?.page ?? 1) < (list.data?.totalPages ?? 1)) setPage((p) => p + 1)
            }}
          >
            {t('expenses.next')}
          </span>
          <span style={{ marginLeft: 12 }}>
            {t('expenses.totalLabel').replace('{amt}', money.format(list.data?.totalAmount ?? 0))}
          </span>
        </div>
      </div>
      ...
      <div className="pay-modal" style={{ width: 380 }}>
        ...
        >
          <p style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 14 }}>
            {t('expenses.markPaidBody')
              .replace('{desc}', expense.description)
              .replace('{amt}', money.format(expense.amount))}
          </p>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
            <Button type="button" variant="soft" onClick={onClose} disabled={run.isPending}>
              {t('expenses.cancel')}
            </Button>
...
function ExpenseFormModal({
  ...
  const [expenseDate, setExpenseDate] = useState(
    expense?.expenseDate ?? new Date().toLocaleDateString('en-CA'),
  )
  ...
  const loadCatOptions = async (q: string) => {
    ...
    return categories
      ...
      .map((c) => ({
        value: c.id,
        label: c.name,
        sublabel: c.isSystem ? t('expenses.systemCat') : `${c.expenseCount ?? 0}`,
      }))
  }
  ...
  const fe = (k: keyof ExpenseFieldErrors): ReactNode =>
    fieldErrors[k] ? <FieldError message={t(fieldErrors[k] as MessageKey)} /> : null
            ...
            {newCat ? (
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <Input
                    value={newCat.name}
                    placeholder={t('expenses.newCatPh')}
                    onChange={(e) => setNewCat({ ...newCat, name: e.target.value })}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault()
                        if (newCat.name.trim()) createCat.mutate()
                      }
                    }}
                  />
                </div>
                ...
                <Button type="button" variant="soft" onClick={() => setNewCat(null)}>
                  {t('expenses.cancel')}
                </Button>
              </div>
            ) : (
              <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <CommandSelect
                    ...
                    onChange={(id) => {
                      setCategoryId(id ?? '')
                      setFieldErrors((x) => ({ ...x, categoryId: undefined }))
                    }}
            ...
            <div className="ff" style={{ marginBottom: 12 }}>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setFieldErrors((x) => ({ ...x, amount: undefined }))
                }}
              />
              {fe('amount')}
            </div>
            <div className="ff" style={{ marginBottom: 12 }}>
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setFieldErrors((x) => ({ ...x, expenseDate: undefined }))
                }}
              />
              {fe('expenseDate')}
            </div>
          ...
          <div className="form-2col">
            ...
            <div className="ff" style={{ marginBottom: 12 }}>
              ...
              {status === 'PENDING' ? (
                <div
                  className="fv"
                  style={{ paddingTop: 9, color: 'var(--text-muted)', fontSize: 12.5 }}
                >
                  {t('expenses.noMethodPending')}
                </div>
              ) : (
                <Select value={method} onChange={(e) => setMethod(e.target.value)}>
                  {PAY_METHODS.map((m) => (
