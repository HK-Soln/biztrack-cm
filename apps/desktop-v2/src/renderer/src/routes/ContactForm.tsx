...
const ID_TYPES: IdDocumentType[] = [
  IdDocumentType.ID_CARD,
  IdDocumentType.PASSPORT,
  IdDocumentType.DRIVERS_LICENSE,
  IdDocumentType.RESIDENCE_PERMIT,
...
export function ContactForm() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const c = editing
        ? await dataClient.contacts.update(id!, payload())
        : await dataClient.contacts.create(payload())
          ...
          const direction =
            type === ContactType.SUPPLIER
              ...
              : type === ContactType.CUSTOMER
                ...
                : obDirection
          await dataClient.openingBalances.upsert({
            contactId: c.id,
            direction,
            amount: amt,
            asOfDate: obAsOfDate,
  ...
  return (
    <div className="frame">
      <BackButton onClick={() => navigate(editing ? `/contacts/${id}` : '/contacts')}>
        {t('ct.title')}
      </BackButton>
      ...
      >
          ...
          <div className="form-2col">
            ...
            <div className="ff" style={{ marginBottom: 12 }}>
              <label className="lbl2">
                {t('ct.name')} <span className="req">*</span>
              </label>
              <Input
                value={name}
                error={!!errors.name}
          ...
          <div className="form-2col">
            ...
            <div className="ff" style={{ marginBottom: 12 }}>
              ...
              <PhoneInput
                ...
                onChange={(v) => {
                  setPhoneAlt(v ?? '')
                  setErrors((x) => ({ ...x, phoneAlt: undefined }))
                }}
              />
        ...
        {!editing ? (
          <div className="card" style={{ marginTop: 14 }}>
            ...
            <div className="form-2col">
              ...
              <div className="ff" style={{ marginBottom: 12 }}>
                ...
                {type === ContactType.BOTH ? (
                  <Select
                    value={obDirection}
                    onChange={(e) => setObDirection(e.target.value as DebtDirection)}
                  >
                    <option value={DebtDirection.RECEIVABLE}>{t('ct.obTheyOwe')}</option>
                    <option value={DebtDirection.PAYABLE}>{t('ct.obYouOwe')}</option>
                  </Select>
                ) : (
                  <div className="fv" style={{ paddingTop: 9 }}>
                    {type === ContactType.SUPPLIER ? t('ct.obYouOwe') : t('ct.obTheyOwe')}
                  </div>
                )}
              </div>
            </div>
            <div className="ff" style={{ maxWidth: 220 }}>
              <label className="lbl2">
                {t('ct.obDate')} <span className="req">*</span>
              </label>
              <Input
                type="date"
                value={obAsOfDate}
                  ...
                  {idDocuments.map((u, i) => (
                    <div key={`${u}-${i}`} className="filechip">
                      ...
                      <button
                        type="button"
                        className="icon-btn"
                        aria-label={t('ct.idDocRemove')}
                        style={{ width: 26, height: 26, marginLeft: 'auto' }}
                        onClick={() => setIdDocuments((d) => d.filter((_, j) => j !== i))}
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
          <Button
            variant="soft"
            type="button"
            onClick={() => navigate(editing ? `/contacts/${id}` : '/contacts')}
            disabled={save.isPending}
          >
            {t('ct.cancel')}
          </Button>
          <Button variant="primary" type="submit" loading={save.isPending}>
            {editing ? t('ct.save') : t('ct.create')}
          </Button>
        </div>
      </form>
    </div>
