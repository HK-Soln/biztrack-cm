...
export function ContactDetail() {
  ...
  const [sendDoc, setSendDoc] = useState<{ html: string; filename: string; title: string } | null>(
    null,
  )
  ...
  if (!contact)
    return (
      <div className="frame">
        <div className="cat-empty">{t('ct.notFound')}</div>
      </div>
    )
  ...
  const oldestDays = contact.oldestUnpaidAt
    ? Math.max(0, Math.floor((Date.now() - new Date(contact.oldestUnpaidAt).getTime()) / 86400000))
    : null
  const metaItem = (icon: ReactNode, value: string) => (
    <span className="mi">
      {icon}
      <span>{value}</span>
    </span>
  ...
  const entryTag = (e: ContactStatementEntry, payable: boolean) => {
    if (e.type === ContactStatementEntryType.OPENING_BALANCE)
      return <span className="et et-woff">{t('debt.entryOpening')}</span>
    if (e.type === ContactStatementEntryType.PAYMENT)
      return <span className="et et-pay">{t('debt.entryPayment')}</span>
    if (e.type === ContactStatementEntryType.WRITE_OFF)
      return <span className="et et-woff">{t('debt.entryWriteOff')}</span>
    return (
      <span className={`et ${payable ? 'et-buy' : 'et-sale'}`}>
        {payable ? t('debt.entryPurchase') : t('debt.entrySale')}
      </span>
    )
  }
  const entryRef = (e: ContactStatementEntry): ReactNode => {
    ...
    if (e.type === ContactStatementEntryType.PAYMENT) {
      // Desktop statements put the method enum in `reference`; the API leaves reference null and
      // puts a ready label in `description`. Handle both so we never render a raw debt.method_null.
      const method = e.reference ? t(`debt.method_${e.reference}` as Parameters<typeof t>[0]) : ''
      const label = method || e.description || t('debt.entryPayment')
      const sub = method ? e.description : ''
      return sub ? (
        <span>
          {label} <span className="sub">· {sub}</span>
        </span>
      ) : (
        label
      )
  ...
  const ledgerView = (stmt: ContactStatement | undefined, footLabel: string) => {
    ...
    // The opening balance is now a proper OPENING_BALANCE entry inside `entries` (with its own
    // payments) — see [[opening-balance-as-debt]]. Render entries directly; do NOT prepend a
    // synthetic opening row, which would show the opening balance twice.
    const all = entries
    ...
    return (
      <>
              ...
              {shown.map((e, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td>{entryRef(e)}</td>
                    <td className="right">
                      {aVal > 0 ? <span className={aClass}>{money.format(aVal)}</span> : '—'}
                    </td>
                    <td className="right">
                      {bVal > 0 ? <span className={bClass}>{money.format(bVal)}</span> : '—'}
                    </td>
                    <td className="right">
                      <span className="t-bal">{money.format(e.balance)}</span>
                    </td>
                  </tr>
                )
              })}
        ...
        <div className="panel-foot">
          ...
          <span>
            {t('ct.closingBalance')}{' '}
            <strong style={{ color: payable ? 'var(--danger)' : 'var(--success)' }}>
              {money.format(stmt?.closingBalance ?? 0)}
            </strong>
          </span>
        </div>
      </>
  ...
  // The opening balance is a proper OPENING_BALANCE entry in `entries` now — no synthetic prepend.
  ...
  const stmtEntryRef = (e: ContactStatementEntry): string => {
    if (e.type === ContactStatementEntryType.OPENING_BALANCE) return t('ct.openingRef')
    if (e.type === ContactStatementEntryType.PAYMENT) {
      // Desktop: method enum in `reference`. API: label in `description`, reference null. Handle both.
      const method = e.reference ? t(`debt.method_${e.reference}` as Parameters<typeof t>[0]) : ''
      const label = method || e.description || t('debt.entryPayment')
      const sub = method ? e.description : ''
  ...
  const statementHtml = (stmt: ContactStatement | undefined, title: string): string => {
    ...
    const rows = stmtRows(stmt)
      .map(
        (e) =>
          `<tr><td>${esc(fmtDay(e.date))}</td><td>${esc(stmtEntryLabel(e, payable))}</td><td>${esc(stmtEntryRef(e))}</td><td class="r">${e.debit ? esc(money.format(e.debit)) : '—'}</td><td class="r">${e.credit ? esc(money.format(e.credit)) : '—'}</td><td class="r">${esc(money.format(e.balance))}</td></tr>`,
      )
      .join('')
  ...
  const sendBtn = (stmt: ContactStatement | undefined, title: string) => (
    <button
      ...
      style={{ height: 34 }}
      onClick={() =>
        setSendDoc({ html: statementHtml(stmt, title), filename: exportName(title), title })
      }
    >
    ...
    <div className="frame">
      <div className="detail-top">
        ...
        <div className="acts2">
          ...
          {isSupplier ? (
            <Button
              variant="soft"
              onClick={() => navigate(`/purchasing/orders/new?supplier=${id}`)}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                <path d="M12 5v14M5 12h14" />
              </svg>
      ...
      <div className="dhero">
        <div className="dhero-in">
          <div className="av round">
            {contact.selfieUrl ? (
              <img src={contact.selfieUrl} alt="" />
            ) : (
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="meta">
              {contact.phone
                ? metaItem(
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                      <path d="M4 5a1 1 0 0 1 1-1h3l2 5-2.5 1.5a11 11 0 0 0 5 5L14 13l3 2v3a1 1 0 0 1-1 1A16 16 0 0 1 4 5Z" />
                    </svg>,
                    contact.phone,
                  )
                : null}
            ...
            <div className="badges">
              ...
              <span className={`st ${contact.isActive ? 'st-brand' : 'st-neutral'}`}>
                <span className="d" />
                {contact.isActive ? t('ct.active') : t('ct.inactive')}
              </span>
            </div>
          </div>
        </div>
      ...
      {isBoth ? (
        <div className="metrics">
          <div className="mc">
            <div className="l">{t('ct.mTheyOwe')}</div>
            <div className="v ok">{money.format(contact.totalReceivable)}</div>
            <div className="s">{t('ct.mReceivableSub')}</div>
          </div>
          <div className="mc">
            <div className="l">{t('ct.mYouOwe')}</div>
            <div className="v bad">{money.format(contact.totalPayable)}</div>
            <div className="s">{t('ct.mPayableSub')}</div>
          </div>
          <div className="mc">
            <div className="l">{t('ct.mNet')}</div>
            <div className={`v ${net >= 0 ? 'ok' : 'bad'}`}>
              {net >= 0 ? '+' : '−'}
              {money.format(Math.abs(net))}
            </div>
            <div className="s">{net >= 0 ? t('ct.mNetFavour') : t('ct.mNetOwe')}</div>
          </div>
          <div className="mc">
            <div className="l">{t('ct.openDebts')}</div>
            <div className="v">{contact.openDebts}</div>
            <div className="s">{t('ct.openDebtsHint')}</div>
          </div>
        </div>
      ) : (
        <div className="metrics">
          <div className="mc">
            ...
            <div className={`v ${isSupplier ? 'bad' : 'ok'}`}>
              {isSupplier ? '−' : '+'}
          ...
          <div className="mc">
            <div className="l">{t('ct.mOldest')}</div>
            <div className={`v ${oldestDays != null && oldestDays > 30 ? 'warn' : ''}`}>
              {oldestDays != null ? t('ct.mOldestDays').replace('{n}', String(oldestDays)) : '—'}
            </div>
            <div className="s">{t('ct.mOldestSub')}</div>
          </div>
          <div className="mc">
            <div className="l">{t('ct.mSince')}</div>
            <div className="v">{since}</div>
            <div className="s">{t('ct.mSinceSub')}</div>
          </div>
        </div>
      )}
          ...
          <div style={{ flex: 1, minWidth: 240 }}>
            ...
            >
              <span style={{ color: 'var(--success)' }}>
                {t('ct.mTheyOwe')} {money.format(contact.totalReceivable)}
              </span>
              <span style={{ color: 'var(--danger)' }}>
                {t('ct.mYouOwe')} {money.format(contact.totalPayable)}
              </span>
            </div>
            <div
              style={{
            ...
            >
              <div
                style={{
                  width: `${Math.round((contact.totalReceivable / Math.max(1, contact.totalReceivable + contact.totalPayable)) * 100)}%`,
            ...
            <div className="hint" style={{ marginTop: 8 }}>
              {(net >= 0 ? t('ct.offsetFavour') : t('ct.offsetOwe')).replace(
                '{v}',
                money.format(Math.abs(net)),
              )}
            </div>
        ...
        <div className="panel mb20">
          <div className="panel-head" style={{ paddingBottom: 0, borderBottom: 0 }}>
            <div className="tabs" style={{ marginBottom: 0, borderBottom: 0 }}>
              <button
                type="button"
                className={ledgerTab === DebtDirection.RECEIVABLE ? 'active' : ''}
                onClick={() => setLedgerTab(DebtDirection.RECEIVABLE)}
              >
                {t('ct.ledgerAsCustomer')}{' '}
                <span className="cnt">{money.compact(contact.totalReceivable)}</span>
              </button>
              <button
                type="button"
                className={ledgerTab === DebtDirection.PAYABLE ? 'active' : ''}
                onClick={() => setLedgerTab(DebtDirection.PAYABLE)}
              >
                {t('ct.ledgerAsSupplier')}{' '}
                <span className="cnt">{money.compact(contact.totalPayable)}</span>
              </button>
            </div>
            <div className="spacer" style={{ flex: 1 }} />
          ...
          {ledgerTab === DebtDirection.RECEIVABLE
            ...
            : ledgerView(payStmt, t('ct.ledgerSuppFoot'))}
        </div>
      ) : (
      ...
      >
        <p style={{ fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.6 }}>
          {t('ct.deleteBody').replace('{name}', contact.name)}
        </p>
        {deleteErr ? (
...
function OffsetModal({
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <Button variant="soft" onClick={onClose} disabled={run.isPending}>
            {t('ct.cancel')}
          </Button>
          <Button
            type="submit"
    ...
    >
      <p style={{ fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.6 }}>
        {t('ct.offsetBody')
          .replace('{amt}', money.format(offsetAmount))
          .replace('{name}', contactName)}
      </p>
      <div style={{ marginTop: 12 }}>
        <div className="recv-tot">
          <span style={{ color: 'var(--text-2)' }}>{t('ct.mTheyOwe')}</span>
          <span>{money.format(receivable)}</span>
        </div>
        <div className="recv-tot">
          <span style={{ color: 'var(--text-2)' }}>{t('ct.mYouOwe')}</span>
          <span>{money.format(payable)}</span>
        </div>
        <div className="recv-tot grand">
          <span>{net >= 0 ? t('ct.offsetAfterFavour') : t('ct.offsetAfterOwe')}</span>
          <span>{money.format(Math.abs(net))}</span>
        </div>
      </div>
      {error ? (
        <p style={{ color: 'var(--danger)', fontSize: 12.5, marginTop: 10 }} role="alert">
  ...
  return (
    <div className="card">
      <div className="card-h">
        <div>
          <h3>{relationship ? t('ct.relationshipProfile') : t('ct.profile')}</h3>
        </div>
      </div>
      <div className="fields-grid">
        <div className="fld">
          <div className="fl">{t('ct.colType')}</div>
          <div className="fv">{typeLabel}</div>
        </div>
        <div className="fld">
          <div className="fl">{t('ct.phone')}</div>
          <div className="fv">{contact.phone || '—'}</div>
        </div>
        <div className="fld">
          <div className="fl">{t('ct.phoneAlt')}</div>
          <div className="fv">{contact.phoneAlt || '—'}</div>
        </div>
        <div className="fld">
          <div className="fl">{t('ct.email')}</div>
          <div className="fv">{contact.email || '—'}</div>
        </div>
        <div className="fld full">
          <div className="fl">{t('ct.address')}</div>
          <div className="fv">{contact.address || '—'}</div>
        </div>
        <div className="fld full">
          <div className="fl">{t('ct.notes')}</div>
          <div className="fv">{contact.notes || '—'}</div>
        </div>
      </div>
    </div>
  )
...
function IdentificationCard({
  ...
  const has = !!(
    ...
    contact.idNumber ||
    contact.idIssueDate ||
    contact.idExpiryDate ||
    contact.idDocuments.length ||
    contact.selfieUrl
  )
    ...
    <div className="card">
      ...
      {!has ? (
        ...
        <>
          <div className="fields-grid">
            <div className="fld">
              <div className="fl">{t('ct.idType')}</div>
              <div className="fv">
                {contact.idType ? t(`ct.idType_${contact.idType}` as Parameters<typeof t>[0]) : '—'}
              </div>
            </div>
            <div className="fld">
              <div className="fl">{t('ct.idNumber')}</div>
              <div className="fv">{contact.idNumber || '—'}</div>
            </div>
            <div className="fld">
              <div className="fl">{t('ct.idIssue')}</div>
              <div className="fv">{fmtDate(contact.idIssueDate)}</div>
            </div>
            <div className="fld">
              <div className="fl">{t('ct.idExpiry')}</div>
              <div className="fv">{fmtDate(contact.idExpiryDate)}</div>
            </div>
          </div>
          {contact.idDocuments.length > 0 ? (
            <div style={{ marginTop: 12 }}>
              <div className="lbl2" style={{ marginBottom: 6 }}>
                {t('ct.idDocs')}
              </div>
