...
export function ReportViewer() {
  ...
  const kpi = {
    sales: useQuery({
      queryKey: ['reports', 'kpi-sales', range],
      queryFn: () => dataClient.sales.summary(range),
      enabled: !routable,
    }),
    expenses: useQuery({
      queryKey: ['reports', 'kpi-exp', range],
      queryFn: () => dataClient.expenses.summary(range),
      enabled: !routable,
    }),
    contacts: useQuery({
      queryKey: ['reports', 'kpi-contacts'],
  ...
  const html = useMemo(
    () =>
      report.data
        ? renderReportDocumentHtml(report.data.document, {
      ...
      return (
        <>
          <header className="m-head">
            {backBtn(() => navigate(-1))}
            <div className="m-tt">
              <div className="m-title">{t('nav.reports')}</div>
              <div className="m-sub">
                {label} · {t('reports.glance')}
              </div>
            </div>
          </header>
          {periodSeg}
          <div className="mkpis" style={{ marginBottom: 18 }}>
            <div className="mkpi">
              <div className="v">{money.compact(revenue)}</div>
              <div className="k">{t('reports.kRevenue')}</div>
            </div>
            <div className="mkpi">
              <div className="v">{money.compact(spend)}</div>
              <div className="k">{t('reports.kExpenses')}</div>
            </div>
            <div className="mkpi">
              <div className="v">{money.compact(revenue - spend)}</div>
              <div className="k">{t('reports.kOperating')}</div>
            </div>
            <div className="mkpi">
              <div className="v">{money.compact(kpi.contacts.data?.totalReceivable ?? 0)}</div>
              <div className="k">{t('reports.kReceivable')}</div>
            </div>
          </div>
              ...
              reps.map((r) => {
                ...
                const inner = (
                  <>
                    ...
                    <div className="mt">
                      <div className="nm">{fr ? r.fr : r.name}</div>
                      <div className="sub">{fr ? r.name : r.fr}</div>
                    </div>
                    {cat.tag ? (
                      <span className="chip-tag">{cat.tag}</span>
                    ) : !on ? (
                      <span className="chip-tag">{t('reports.soonTag')}</span>
                    ) : null}
                    {on ? (
                      <svg
                        className="chev"
                ...
                return on ? (
                  <button key={r.id} type="button" className="mrow" onClick={() => select(r.id)}>
                    {inner}
                  </button>
                ) : (
                  <div key={r.id} className="mrow" style={{ opacity: 0.55, cursor: 'default' }}>
                    {inner}
                  </div>
                )
              }),
    ...
    return (
      <>
        <header className="m-head">
          ...
          <div className="m-tt">
            <div className="m-title">{meta ? (fr ? meta.fr : meta.name) : t('nav.reports')}</div>
            <div className="m-sub">{label}</div>
          </div>
        </header>
        {periodSeg}
        {routable ? (
          <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
            ...
            >
              {I.pdf}
              {busy ? '…' : t('reports.exportPdf')}
            </button>
          </div>
        ) : null}
        {!routable ? (
          <div className="mcard">
            <p className="hint">{t('reports.soonBody')}</p>
          </div>
        ) : report.isError ? (
          <div className="mcard">
            <p className="hint">{t('reports.loadErrorBody')}</p>
          </div>
        ) : report.isPending || !html ? (
          <div className="mcard">
            <p className="hint">{t('reports.loading')}</p>
          </div>
        ) : (
          <div className="rv-paper" style={{ height: '72vh' }}>
            <iframe ref={iframeRef} srcDoc={html} title={meta ? meta.name : 'report'} />
          </div>
        )}
      </>
    )
  ...
  return (
    <>
      <div className="rv-layout">
        ...
        <div className="rv-picker">
          ...
          <button
            type="button"
            className="rv-item"
            aria-current={!id ? 'true' : 'false'}
            onClick={() => navigate(`/reports?period=${period}`)}
          >
            <span className="nm">
              {t('reports.overview')}
              <small>{t('reports.overviewSub')}</small>
            </span>
          </button>
              ...
              {reps.map((r) => {
                ...
                return (
                  <button
                    ...
                    className="rv-item"
                    disabled={!on}
                    aria-current={r.id === id ? 'true' : 'false'}
                    onClick={() => select(r.id)}
                    style={!on ? { cursor: 'default' } : undefined}
                  >
                    <span className="nm">
                      {fr ? r.fr : r.name}
                      <small>{fr ? r.descFr : r.desc}</small>
                    </span>
                    <span className={`fmt${on ? '' : ' soon'}`}>
                      {on ? r.formats.map((f) => f.toUpperCase()).join('·') : t('reports.soonTag')}
                    </span>
                  </button>
                )
              })}
            ...
            {!id ? (
              <div className="rv-overview">
                ...
                <div className="grid4 mb20">
                  <div className="kpi">
                    <div className="lab">{t('reports.kRevenue')}</div>
                    <div className="val">{money.compact(revenue)}</div>
                    <div className="hint">
                      {t('reports.hSales').replace(
                        '{n}',
                        String(kpi.sales.data?.transactions ?? 0),
                      )}
                    </div>
                  </div>
                  <div className="kpi">
                    <div className="lab">{t('reports.kExpenses')}</div>
                    <div className="val">{money.compact(spend)}</div>
                    <div className="hint">
                      {t('reports.hEntries').replace('{n}', String(kpi.expenses.data?.count ?? 0))}
                    </div>
                  </div>
                  <div className="kpi">
                    <div className="lab">{t('reports.kOperating')}</div>
                    <div className="val">{money.compact(revenue - spend)}</div>
                    <div className="hint">{t('reports.hOperating')}</div>
                  </div>
                  <div className="kpi">
                    <div className="lab">{t('reports.kReceivable')}</div>
                    <div className="val">
                      {money.compact(kpi.contacts.data?.totalReceivable ?? 0)}
                    </div>
                    <div className="hint">
                      {t('reports.hDebtors').replace(
                        '{n}',
                        String(kpi.contacts.data?.debtorCount ?? 0),
                      )}
                    </div>
                  </div>
                </div>
                <div className="rv-soon">
                  ...
                  <h3>{t('reports.pickTitle')}</h3>
                  <p>{t('reports.pickBody')}</p>
                </div>
              </div>
            ) : !routable ? (
              <div className="rv-soon">
                <div className="ic">{I.doc}</div>
                <h3>{t('reports.soonTitle')}</h3>
                <p>{t('reports.soonBody')}</p>
              </div>
            ) : report.isError ? (
              <div className="rv-soon">
                <h3>{t('reports.loadError')}</h3>
                <p>{t('reports.loadErrorBody')}</p>
              </div>
            ) : report.isPending || !html ? (
              <div className="rv-soon">
      ...
      {fs && html ? (
        <div className={`rv-fs${isElectron ? ' desktop' : ''}`} ref={fsRef}>
          <div className="rv-fs-bar">
            <div className="rv-fs-title">
              {meta ? (fr ? meta.fr : meta.name) : ''} · {label}
            </div>
            {/* Hover-revealed big X — F11/Esc aren't discoverable, so surface a clear way out. */}
            <button
              type="button"
              className="rv-fs-exit"
              onClick={closeFs}
              title={t('reports.exitFs')}
              aria-label={t('reports.exitFs')}
            >
              {I.x}
            </button>
          ...
          <div className="rv-fs-body">
            <iframe ref={fsIframeRef} srcDoc={html} title="report-fullscreen" />
          </div>
        </div>
      ) : null}
    </>
