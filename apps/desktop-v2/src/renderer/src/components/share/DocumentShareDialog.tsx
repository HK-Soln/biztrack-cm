...
export function DocumentShareDialog({
  ...
  const download = useMutation({
    ...
    onSuccess: (r) => {
      if (r.saved) setDone(t('share.downloaded'))
    },
    onError: (e) => setError(errorMessage(e, t('share.downloadError'))),
  })

  // Offline: opening WhatsApp/email server-side can't reach the server, so save the PDF
  // instead — the owner can attach it manually. Not a dead end.
  const saveOffline = async () => {
    setError(null)
    try {
      const r = await download.mutateAsync()
      if (r.saved) setDone(t('share.offlineDownloaded'))
    } catch (e) {
      setError(errorMessage(e, t('share.downloadError')))
    }
    setMode('menu')
  }
  ...
  // Email/WhatsApp now ask the SERVER to deliver the statement as a PDF (Resend / WAHA);
  // the client no longer opens a local composer.
  const share = useMutation({
    ...
    onSuccess: () => {
      setDone(t('share.sent'))
      setMode('menu')
    },
    onError: (e) => {
      // Lost the connection mid-send → fall back to download rather than fail outright.
      if (isOffline()) return void saveOffline()
      setError(errorMessage(e, t('share.error')))
    },
  })
  const submitSend = () => {
    if (mode === 'menu') return
  ...
  const optionRow = (icon: ReactNode, label: string, sub: string, onClick: () => void) => (
    <button type="button" className="share-opt" onClick={onClick}>
      ...
      <span className="share-opt-main">
        <span className="share-opt-lab">{label}</span>
        <span className="share-opt-sub">{sub}</span>
      </span>
    </button>
  )
    ...
    >
      <div className="pay-modal" style={{ width: 440 }}>
        <div className="pm-head">
          <h3>
            {mode === 'menu' ? title : mode === 'whatsapp' ? t('share.whatsapp') : t('share.email')}
          </h3>
          <button
            type="button"
            className="x"
        ...
        <div className="pm-body">
          {mode === 'menu' ? (
            <>
              {done ? (
                <p style={{ color: 'var(--success)', fontSize: 13, marginBottom: 10 }}>{done}</p>
              ) : null}
            ...
            <>
              <p style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 12 }}>
                {t('share.toSupplier').replace('{name}', recipientName)}
              </p>
              {mode === 'whatsapp' ? (
                <div className="ff">
