  ...
  return (
    <div className="receipt">
      ...
      <div className="receipt-act">
        <button type="button" disabled={printing} onClick={() => void print()}>
          {I.print}
          {printing ? '…' : t('sales.print')}
        </button>
        <button type="button" className="primary" onClick={() => setSendOpen(true)}>
          {I.send}
          {t('sales.sendReceipt')}
        </button>
      </div>
      ...
      {note ? (
        <div className="hint" style={{ textAlign: 'center', padding: '0 18px 14px' }}>
          {note}
        </div>
      ) : null}
      {sendOpen ? (
