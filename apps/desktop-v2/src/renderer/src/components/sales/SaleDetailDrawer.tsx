 ...
 */
export function SaleDetailDrawer({ saleId, onClose }: { saleId: string | null; onClose: () => void }) {
...
function SaleDetailBody({ saleId, onClose }: { saleId: string; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="drawer-b">
        <div className="ramt" style={{ fontSize: 26, fontWeight: 700, letterSpacing: '-.5px', fontVariantNumeric: 'tabular-nums' }}>
          {money.format(sale.totalAmount)}
        </div>
        <div className="rmeta" style={{ display: 'flex', flexWrap: 'wrap', gap: 8, margin: '12px 0 18px', alignItems: 'center' }}>
        ...
        <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--text-muted)', margin: '4px 0 6px' }}>
        ...
        {note ? <div className="hint" style={{ textAlign: 'center', paddingTop: 10 }}>{note}</div> : null}
      </div>

      <div className="drawer-f">
        <button type="button" className="btn" disabled={printing} onClick={() => void print()} style={{ flex: 1, justifyContent: 'center' }}>
          {I.print}
          {printing ? '…' : t('sales.print')}
        </button>
        <button type="button" className="btn btn-primary" onClick={() => setSendOpen(true)} style={{ flex: 1, justifyContent: 'center' }}>
          {I.send}
          {t('sales.sendReceipt')}
        </button>
      </div>
      ...
      {sendOpen ? (
        <ReceiptSendDialog sale={sale} customerName={custName} locale={lang} onClose={() => setSendOpen(false)} />
      ) : null}
    </>
  )
}
