...
/** Just the bits of a sale the send flow needs — works for LocalSale and LocalSaleDetail. */
type ReceiptSendSale = Pick<LocalSale, 'id' | 'customerId' | 'customerPhone'>

const X = <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M6 6l12 12M18 6 6 18" /></svg>
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/

/**
 * Send a receipt to the customer — same UX as the RFQ/PO ShareDialog: a menu (Email /
 * WhatsApp / Download) then a recipient step. Online → the server dispatches; offline → the
 * desktop WhatsApp/email composer opens with the PDF revealed. Download always works.
 * Shared by the Sell success screen and the Sales receipt detail pane.
 */
export function ReceiptSendDialog({ sale, customerName, locale, onClose }: { sale: ReceiptSendSale; customerName: string; locale: string; onClose: () => void }) {
  const t = useT()
  ...
  // Rendered on the .pay-overlay chrome (z-index 300) so it stacks above any host modal.
  return (
