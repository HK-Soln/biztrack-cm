...
export function UserSecuritySection() {
  ...
  const flash = (m: string) => {
    setToast(m)
    window.setTimeout(() => setToast((c) => (c === m ? null : c)), 2400)
  }
  ...
  async function savePin() {
    ...
    setPinBusy(true)
    try {
      await dataClient.pin.set(pin)
      setPin('')
      setPinConfirm('')
      flash(t('sec.pinSaved'))
    } catch {
      flash(t('sec.pinError'))
    } finally {
      setPinBusy(false)
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Manager PIN — a real, wired credential (unlike the prototype cards below). */}
      {canManagePin ? (
        <div className="card">
          <div className="card-h">
            <div>
              <h3>{t('sec.pinTitle')}</h3>
              <p>{t('sec.pinSub')}</p>
            </div>
          </div>
          ...
          <div className={`ff${pinMismatch ? ' invalid' : ''}`} style={{ marginTop: 12 }}>
            ...
            {pinMismatch ? (
              <div className="msg err">
                ...
                <span>{t('sec.pinMismatch')}</span>
              </div>
            ) : null}
          </div>
          <div className="form-note">
            <Info />
            <span>{t('sec.pinOnlineNote')}</span>
          </div>
      ...
      <div className="card">
        ...
        <div className="ff">
          <label className="lbl2">{t('sec.current')}</label>
        ...
        <div className={`ff${mismatch ? ' invalid' : ''}`} style={{ marginTop: 12 }}>
          ...
          {mismatch ? (
            <div className="msg err">
              ...
              <span>{t('sec.pwMismatch')}</span>
            </div>
          ) : null}
        </div>
        <div className="fp-actions" style={{ marginTop: 16 }}>
          <Button variant="primary" type="button" disabled={!pwValid} onClick={savePw}>
            {t('sec.updatePw')}
          </Button>
        </div>
      </div>
        ...
        <div className="set-line">
          <div>
            <div className="nm" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ color: twofa ? 'var(--success)' : 'var(--text-muted)' }}>
                <Shield />
              </span>
          ...
          {twofa ? (
            <Button
              ...
              onClick={() => {
                setTwofa(false)
                flash(t('sec.2faDisabled'))
              }}
            >
              {t('sec.disable')}
            </Button>
          ) : setup ? null : (
            <Button variant="primary" type="button" onClick={() => setSetup(true)}>
              {t('sec.setup')}
            </Button>
          )}
        </div>
            ...
            <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap', alignItems: 'center' }}>
              <div
                style={{
                  ...
                  borderRadius: 10,
                  background:
                    'conic-gradient(from 0deg,#000 25%,#fff 0 50%,#000 0 75%,#fff 0) 0 0/24px 24px, #fff',
              ...
              <div style={{ minWidth: 220, flex: 1 }}>
                ...
                <div
                  className="tcode"
                  style={{ display: 'inline-block', marginTop: 4, letterSpacing: '.08em' }}
                >
                  {SECRET}
                </div>
              </div>
