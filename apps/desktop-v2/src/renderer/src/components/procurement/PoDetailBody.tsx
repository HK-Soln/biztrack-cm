...
/** Purchase-order detail content (header + items + actions), shared by the PoDetail
 * route and the PO list drawer. Fetches the PO by id; renders no page chrome. */
export function PoDetailBody({ id }: { id: string }) {
  const t = useT()
  ...
  // "Create & send" lands on the detail route with state.share → open share once.
  useEffect(() => {
  ...
  if (isPending) return <div className="cat-empty">{t('po.loading')}</div>
  if (!po) return <div className="cat-empty">{t('po.notFound')}</div>
    ...
    <>
      <div className="page-head">
        <div>
          <h1>
            {po.number}{' '}
            <span className={`st ${STATUS_CLASS[po.status] ?? 'st-neutral'}`}>
              {t(`po.status_${po.status}` as Parameters<typeof t>[0])}
            </span>
          </h1>
          <p>
            {po.supplierName ?? '—'}
            {po.expectedDate
              ? ` · ${t('po.expectedShort')} ${new Date(po.expectedDate).toLocaleDateString()}`
              : ''}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {canReceive ? (
            <Button
              variant="primary"
              onClick={() => navigate(`/purchasing/orders/${po.id}/receive`)}
            >
              {t('po.receive')}
            </Button>
          ) : null}
          ...
          <ActionMenu
            items={[
              ...
              ...(canCancel
                ? [{ label: t('po.cancelPo'), danger: true, onClick: () => cancelM.mutate() }]
                : []),
            ]}
          />
        </div>
      ...
      <div className="card">
        ...
        <div className="totals" style={{ marginLeft: 'auto', width: 260, marginTop: 8 }}>
          ...
          >
            ...
            <span>{money.format(po.totalAmount)}</span>
          </div>
        </div>
        {po.messageBody ? (
