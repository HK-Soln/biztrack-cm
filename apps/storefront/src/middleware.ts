...
export function middleware(req: NextRequest) {
  // Payment links (/pay/[token]) and digital receipts (/r/[saleId]) are store-agnostic — they must
  // render on ANY host, including the root, so never bounce them to marketing.
  if (req.nextUrl.pathname.startsWith('/pay/') || req.nextUrl.pathname.startsWith('/r/'))
