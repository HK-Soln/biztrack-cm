...
export type LocalDeposit = CustomerDeposit & {
  depositCount: number
  lastTransactionAt: string | null
}

export type LocalDepositStatement = Omit<DepositStatement, 'account'> & {
  account: LocalDeposit
}
...
function assertBusinessId(businessId: string | null | undefined): asserts businessId is string {
  if (!businessId) {
    throw new DepositLocalError('DEPOSIT_CUSTOMER_REQUIRED', 'Business ID is required')
  }
}
...
function rowToDeposit(
  row: SavingsAccountRow,
  transactionCount = 0,
  lastTxAt: string | null = null,
): LocalDeposit {
  let taggedProducts: DepositTaggedProduct[] | null = null
  if (row.tagged_products) {
    try {
      taggedProducts = JSON.parse(row.tagged_products) as DepositTaggedProduct[]
    } catch {
      taggedProducts = null
    }
  }
...
function buildAccountSyncPayload(account: SavingsAccountRow, now: string) {
  return {
    ...
    taggedProducts: account.tagged_products ? (JSON.parse(account.tagged_products) as DepositTaggedProduct[]) : null,
...
export async function createOrDepositLocal(
  businessId: string,
  input: CreateDepositInput & { actorId?: string | null },
): Promise<{ accountId: string; transactionId: string; wasCreated: boolean }> {
  assertBusinessId(businessId)

  if (!input.customerId) {
    throw new DepositLocalError('DEPOSIT_CUSTOMER_REQUIRED', 'Customer ID is required')
  }

  const { amount, method } = input.initialDeposit

  if (!amount || amount <= 0) {
    throw new DepositLocalError('DEPOSIT_AMOUNT_INVALID', 'Deposit amount must be greater than zero')
  }
  ...
  if (existing.length > 0) {
    ...
    const tx = await createDepositTransactionLocal(businessId, accountId, {
...
export async function createDepositTransactionLocal(
  businessId: string,
  savingsId: string,
  input: CreateDepositTransactionInput & { actorId?: string | null },
): Promise<{ id: string }> {
  assertBusinessId(businessId)
  ...
  if (!amount || amount <= 0) {
    throw new DepositLocalError('DEPOSIT_AMOUNT_INVALID', 'Transaction amount must be greater than zero')
  }
  ...
  if (!accountRows.length) {
    throw new DepositLocalError('DEPOSIT_NOT_FOUND', 'Deposit account not found')
  }
  ...
  if (!isInbound && safeAmount > account.balance) {
    throw new DepositLocalError('DEPOSIT_INSUFFICIENT_BALANCE', 'Transaction amount exceeds account balance')
  }
...
export async function recordDepositLocal(
  businessId: string,
  savingsId: string,
  ...
  return createDepositTransactionLocal(businessId, savingsId, {
    type: 'deposit',
    direction: 'inbound',
    amount: input.amount,
    method: input.method ?? null,
...
export async function recordDepositRefundLocal(
  businessId: string,
  savingsId: string,
  input: { amount: number; method?: string | null; mobileMoneyReference?: string | null; notes?: string | null; recordedById?: string | null; actorId?: string | null },
): Promise<{ id: string }> {
...
export async function recordDepositUsageLocal(
  businessId: string,
  savingsId: string,
  saleId: string,
  amount: number,
  ...
  const safeAmount = roundMoney(amount)
  if (safeAmount <= 0) {
    throw new DepositLocalError('DEPOSIT_AMOUNT_INVALID', 'Usage amount must be greater than zero')
  }
...
export async function getDepositAccountLocal(
  businessId: string,
  savingsId: string,
): Promise<LocalDeposit> {
  assertBusinessId(businessId)
  ...
  const row = rows[0]!
  return rowToDeposit(row, row.transaction_count ?? 0, row.last_transaction_at ?? null)
}

export async function getDepositAccountByCustomerLocal(
  businessId: string,
  customerId: string,
): Promise<LocalDeposit | null> {
  assertBusinessId(businessId)
...
export async function getDepositStatementLocal(
  businessId: string,
  savingsId: string,
): Promise<LocalDepositStatement> {
  assertBusinessId(businessId)
  ...
  const entries = txRows.map((tx) => {
    ...
    return {
      id: tx.id,
      type: tx.type as DepositTransactionType,
      direction: tx.direction as DepositTransactionDirection,
      amount: tx.amount,
      method: tx.method,
  ...
  return {
    account: rowToDeposit(accountRow, depositCount, lastTxAt),
    entries,
  }
}

export async function listDepositsLocal(
  businessId: string,
  query: DepositsQuery = {},
): Promise<PaginatedResult<LocalDeposit>> {
  assertBusinessId(businessId)
  ...
  return {
    data: pageRows.map((row) => rowToDeposit(row, row.transaction_count ?? 0, row.last_transaction_at ?? null)),
    total,
    page: safePage,
    limit: safeLimit,
    totalPages,
...
// ---------------------------------------------------------------------------
// Backwards-compat re-exports — keep until all consumers are migrated
// ---------------------------------------------------------------------------
export type LocalSavingsAccount = LocalDeposit
export type LocalSavingsStatement = LocalDepositStatement
export type SavingsErrorCode = DepositErrorCode
export { DepositLocalError as SavingsLocalError }
export const createOrDepositSavingsLocal = createOrDepositLocal
export const getSavingsAccountLocal = getDepositAccountLocal
export const getSavingsAccountByCustomerLocal = getDepositAccountByCustomerLocal
export const getSavingsStatementLocal = getDepositStatementLocal
export const listSavingsAccountsLocal = listDepositsLocal
export const recordSavingsDepositLocal = recordDepositLocal
export const recordSavingsRefundLocal = recordDepositRefundLocal
export const recordSavingsUsageLocal = recordDepositUsageLocal
