...
import { DepositRefundDialog } from '@/components/deposits/DepositRefundDialog'
import { cn } from '@/lib/utils'
import { hasDesktopIpc, ipc } from '@/services/ipc.bridge'
import {
  getDepositStatementLocal,
  type LocalDepositStatement,
} from '@/services/deposits.local'
...
export default function DepositsDetailPage() {
  const t = useTranslations('app.deposits')
  ...
  const [statement, setStatement] = useState<LocalDepositStatement | null>(null)
  ...
  useEffect(() => {
    if (!businessId || !depositId) {
      setLoading(false)
      setError(t('errors.DEPOSIT_NOT_FOUND'))
      return
    }
...
<html>
<head><meta charset="utf-8"><title>Customer Deposit Statement – ${acc.accountNumber}</title></head>
<body style="margin:0;padding:32px;font-family:Arial,sans-serif;color:#111827;background:#fff;">
  <div style="margin-bottom:24px;padding-bottom:20px;border-bottom:2px solid #e5e7eb;">
    ...
    <div style="font-size:13px;color:#6b7280;margin-top:4px;">Customer Deposit Statement</div>
  </div>
  ...
  ${
    ...
    <tbody>${rows}</tbody>
      ...
      await ipc.share.file({
        ...
        filename: `deposit-statement-${acc.accountNumber}.pdf`,
  ...
  if (error || !statement) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3">
        <p className="text-sm text-destructive">{error ?? t('errors.DEPOSIT_NOT_FOUND')}</p>
        <Button variant="outline" size="sm" onClick={() => router.push(`/${locale}/deposits`)}>
          {t('detail.back')}
        </Button>
      </div>
    )
    ...
    <div className="flex h-full flex-col gap-6 overflow-auto p-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <button
          ...
          onClick={() => router.push(`/${locale}/deposits`)}
        >
          ← {t('detail.back')}
        </button>
