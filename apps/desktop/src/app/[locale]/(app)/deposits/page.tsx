...
export default function DepositsPage() {
  ...
  const [accounts, setAccounts] = useState<PaginatedResult<LocalDeposit> | null>(null)
  ...
  const [depositAccount, setDepositAccount] = useState<LocalDeposit | null>(null)

  const [refundOpen, setRefundOpen] = useState(false)
  const [refundAccount, setRefundAccount] = useState<LocalDeposit | null>(null)
  ...
  useEffect(() => {
    ...
    listDepositsLocal(businessId, { page, limit: PAGE_SIZE, search })
  ...
  function openDeposit(account?: LocalDeposit) {
    setDepositAccount(account ?? null)
    setDepositOpen(true)
  }

  function openRefund(account: LocalDeposit) {
              ...
              {accounts.data.map((account) => (
                ...
                >
                  <td className="py-3 pr-4 font-mono text-xs text-foreground">
                    <Link
                      href={`/deposits/detail?depositId=${account.id}`}
                      ...
                      items={[
                        {
                          ...
                          onSelect: () =>
                            router.push(`/${locale}/deposits/detail?depositId=${account.id}`),
                        },
                        {
