...
export function DepositDialog({
  ...
  function resolveErrorMessage(err: unknown): string {
    if (err instanceof DepositLocalError) {
      try {
        return t(`errors.${err.code as DepositErrorCode}` as Parameters<typeof t>[0])
      } catch {
        return err.message
      }
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!account && !selectedContact) {
      toast.error(t('errors.DEPOSIT_CUSTOMER_REQUIRED'))
      return
    }
    if (!parsedAmount || parsedAmount <= 0) {
      toast.error(t('errors.DEPOSIT_AMOUNT_INVALID'))
      return
    }
    ...
    try {
      if (account) {
        await recordDepositLocal(businessId, account.id, {
          amount: parsedAmount,
          method,
        ...
        await createOrDepositLocal(businessId, {
