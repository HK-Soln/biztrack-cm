...
function buildBizDetails(input: BaseTemplateInput): string {
  const parts: string[] = []
  if (input.businessLegalForm) parts.push(escapeHtml(input.businessLegalForm))
  if (input.businessAddress) parts.push(escapeHtml(input.businessAddress))
  if (input.businessPhone) parts.push(escapeHtml(input.businessPhone))
  if (input.businessEmail) parts.push(escapeHtml(input.businessEmail))
  if (input.businessRccm) parts.push(`RCCM: ${escapeHtml(input.businessRccm)}`)
  if (input.businessNiu) parts.push(`NIU: ${escapeHtml(input.businessNiu)}`)
  return parts.length > 0 ? parts.join(' · ') : 'BizTrack CM'
}
    ...
    <div class="paper">
      <div class="letterhead">
        <div>
          ...
          <p class="biz-details">${buildBizDetails(input)}</p>
        </div>
