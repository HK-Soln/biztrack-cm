...
7. **Sync-freshness guard (data-derived producers)** — the API computes digests/alerts over _synced_ data, so if a business's device hasn't synced recently the server's view is stale and a digest/alert would be **wrong**. Every data-derived producer (stock alerts P1, daily digest P2, receivables P3) must first check freshness: `now − MAX(sync_device_sessions.last_used_at for the business) > threshold`. If stale, **skip the data notification and instead send a "your data is old — please sync" alert** (in-app, to the owner). This is a system alert, not one of the 7 configurable events (it shouldn't be silenceable — it explains why the digests stopped). A shared `SyncFreshnessGuard` helper lands with the first data producer (P1) and is reused by P2/P3.
8. **Deeplink → full-URL resolution (shared route registry)** — a notification's deeplink is fine as a **short path** (`/inventory`) for the **in-app bell** (the app routes it), but **email / WhatsApp / SMS open outside the app**, so those bodies need a **full URL** (`https://<web-app>/inventory`). That requires (a) the backend to know the frontend **base URL** (env, pointing at the cloud/web build — not the Electron app), and (b) the route paths to move into a **shared registry** (a package importable by both `apps/desktop-v2` for routing and `apps/api` for building full URLs), so the two never drift. This is a cross-cutting refactor that touches the desktop router + all navigation call sites, so it is **planned separately** (see "Deeplinks & URLs" below) — until it lands, external-channel bodies omit the link. Owner-flagged 2026-08-20.
...
## Deeplinks & URLs (planned separately — affects the frontend broadly)

**Problem.** In-app notifications navigate by short path; external channels (email /
WhatsApp / SMS) are opened outside the app and need an absolute URL. Today deeplinks are
hardcoded short strings at the producer (`/inventory`), and route paths live inline in the
desktop router — so there's no single source of truth and no way for the backend to build
a correct external URL.

**Target design.**

1. **Shared route registry** — a small package (e.g. `packages/routes`, or a `routes`
   module in `packages/types`) exporting every frontend path as data/functions:
   `ROUTES.inventory`, `ROUTES.reorder`, `ROUTES.debt(id)`, … Pure strings/builders, no
   React. Consumed by:
   - **Frontend** — the desktop-v2 router + all `navigate(...)`/`<Link>` call sites use
     the registry instead of inline literals (migrated incrementally; the registry can
     land first and callers move over).
   - **Backend** — producers build deeplinks from the registry so the path is identical
     to what the app routes.
2. **Base URL config (backend)** — a validated env var (e.g. `WEB_APP_URL`) pointing at
   the **cloud/web** build's origin (the URL a customer's browser can open — not the
   Electron app). The API composes `fullUrl = WEB_APP_URL + ROUTES.x`.
3. **Dispatcher/notification shape** — keep the short `deeplink` for the in-app row;
...
| N6  | Sync-freshness threshold + stale-sync alert            | P1+ (data producers)       | Data-derived producers guard on `MAX(sync_device_sessions.last_used_at)`; if `now − last_sync > threshold` (propose 36h) skip the digest and instead send an always-on in-app "please sync" alert to the owner. Threshold + whether the stale alert is fully non-silenceable to confirm.                                                                                                                                                       |
| N7  | Deeplink → full-URL (shared route registry + base URL) | external-channel producers | Where the shared route registry lives (`packages/routes` vs a `routes` module in `packages/types`); the base-URL env var name + which origin it points at (cloud/web build); embed the full URL in the external body vs a structured `url` field; big-bang vs incremental migration of the desktop router. See "Deeplinks & URLs".                                                                                                             |
